...
用法（在 someopark-test/ 目录下）：
    ...
    python qlib-main/sector_rotation/backtest/trade_audit.py [--output-dir DIR]

2026-07-22(镜像 AISS): 全体包进 main() + __main__ 守卫——此前模块顶层执行,
import 即跑全量审计并覆写 historical_runs/ 的 csv(import 副作用地雷)。
新增 --output-dir(默认仍是 historical_runs/,无参调用行为不变)。
"""
...
def main(output_dir: str | None = None) -> None:
    ...
    from sector_rotation.backtest.engine    import SectorRotationBacktest  # noqa: F401
    ...
    # ── 2. Save to CSV ────────────────────────────────────────────────────────
    _out_dir = output_dir or os.path.join(REPO_ROOT, "historical_runs")
...
if __name__ == "__main__":
    import argparse
    _ap = argparse.ArgumentParser(description="SSRS trade audit replay")
    _ap.add_argument("--output-dir", default=None,
                     help="CSV output dir (default: historical_runs/)")

"""macro_clusters — SSRS Layer-1 宏观定位的离线重建(2026-08-16 真修 B)。
...
历史病灶(三层,weekly success-degraded 根因):
  ...
  2) 私掏 API: smart_select/SectorRotationBatchRun 直塞**生向量**进 _encode,跳过
     prepass/log/标准化 —— 即便在旧代码下,latent 也是尺度噪音;
  3) 基底漂移: serving 每日惰性重训 autoencoder,latent 基底随数据增长漂移,
     与离线 centroids 比距离无意义。

真修(本模块 = 单一事实源,standalone CLI 与 SectorRotationBatchRun 共用):
...
用法(qlib_run 环境):
    python -m sector_rotation.macro_clusters            # 读 wf_fold_detail.json 重建
    python -m sector_rotation.macro_clusters --dry-run  # 只训练+聚类,不落盘
"""
...
log = logging.getLogger("sector_rotation.macro_clusters")

...
def _sync_cost_basis_from_ledger(inv: dict, snap_path=None) -> int:
    ...
    """
    try:
        acct_path = Path(__file__).resolve().parent / "account_ssrs.json"
        ...
        return 0
    n = 0
    for tk, h in (inv.get("holdings") or {}).items():
        p = pos.get(tk)
        if not p:
            continue
...
def run_daily_signal(
    ...
    # when per-sector scores are stable) so the NEW param's optimizer weights are
    # actually adopted.  Without this, a param/optimizer switch would leave the
    # book stuck on the old allocation whenever signals move < threshold.
    # (inv["param_set"]/["signal_version"] still hold the PREVIOUS run's values at
    # this point — set at end of run — so this compares new-vs-previous.)

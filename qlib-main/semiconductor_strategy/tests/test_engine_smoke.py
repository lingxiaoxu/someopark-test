...
Exercises the complete pipeline:
    ...
    → AISSWeightStrategy (WeightStrategyBase)
...
from semiconductor_strategy.backtest.engine import AISSBacktest, BacktestResult
...
_TICKERS = ["ai_gpu", "custom_asic", "equipment", "memory_hbm",
            "foundry", "analog_defense", "logic_cpu", "rf_edge"]  # AISS subsector assets
_BENCH = "SPY"
_START = "2020-01-01"
_END   = "2021-06-30"
...
class TestEngineSmokeNative(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        from semiconductor_strategy.backtest import engine as _eng
...
class TestBacktestResultFields(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        prices = _make_prices()
        macro  = _make_macro()
        bt = AISSBacktest(_minimal_config(), mlflow_experiment="semiconductor_strategy_test")
        cls.result = bt.run(prices=prices, macro=macro)
    ...
    def test_summary_runs_without_error(self):
        ...
        self.assertIn("AISS (AI Infra & Semiconductor) BACKTEST SUMMARY", s)

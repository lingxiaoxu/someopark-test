...
from semiconductor_strategy.backtest.costs import (
...
from semiconductor_strategy.backtest.metrics import (
    annualized_return,
    annualized_vol,
    sharpe_ratio,
    max_drawdown,
...
class TestTransactionCosts(unittest.TestCase):
    ...
    def test_cost_bps_by_tier(self):
        self.assertEqual(get_one_way_cost_bps("NVDA"), 3)  # Tier 1 (mega-cap)
        self.assertEqual(get_one_way_cost_bps("KLAC"), 5)  # Tier 2 (large-cap)
        self.assertEqual(get_one_way_cost_bps("CRDO"), 8)  # Tier 3 (small/IPO)
...
class TestExpectedMaxSharpe(unittest.TestCase):
    ...
    def _f(self):
        from semiconductor_strategy.walk_forward import expected_max_sharpe

"""
aiss_verify_excel.py — deep per-sheet verification of AISS backtest Excel.
=========================================================================
Opens EVERY sheet of EVERY AISS portfolio Excel and runs computation
...
Usage:
    conda run -n qlib_run python -m semiconductor_strategy.tests.aiss_verify_excel [YYYYMMDD] [--dir PATH]
...
from semiconductor_strategy.backtest.metrics import sharpe_ratio as _ci_sharpe

DEFAULT_DIR = os.path.abspath(os.path.join(
    _HERE, "..", "..", "..", "historical_runs", "semiconductor_strategy"))
...
def main():
    ap = argparse.ArgumentParser(description="Deep per-sheet verification of AISS backtest Excel")
    ap.add_argument("date", nargs="?", default=None, help="YYYYMMDD (default: today)")
    ap.add_argument("--dir", default=DEFAULT_DIR, help="historical_runs/semiconductor_strategy dir")
    args = ap.parse_args()
    D = args.date or pd.Timestamp.today().strftime("%Y%m%d")

    out = [f"AISS Excel deep verification — date={D} dir={args.dir}"]
    total_bad = 0; total_files = 0
    for pat, lab in [
        (f"aiss_portfolio_*_v1_IS_batch_{D}_*.xlsx",        "V1 IS batch"),
        (f"aiss_portfolio_*_v2_IS_batch_{D}_*.xlsx",        "V2 IS batch"),
        (f"aiss_portfolio_*_v1_IS-OOS_tearsheet_{D}_*.xlsx","V1 IS-OOS"),
        (f"aiss_portfolio_*_v2_IS-OOS_tearsheet_{D}_*.xlsx","V2 IS-OOS"),
    ]:

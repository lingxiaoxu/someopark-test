...
AISS Data Loader
================
Loads the AI Infra & Semiconductor Strategy's data and constructs the **subsector
basket price series** that are the strategy's tradeable assets.

Three layers
------------
1. Single-stock prices  — read from the isolated AISS price store
   (``price_data/semi_strategy/prices/`` via ``aiss_fetch_prices``).
2. Subsector baskets    — 8 synthetic 80/15/5 total-return indices built from the
   ...
   (SOXX/SMH/SPY) are what the signals / optimizer / engine consume — exactly the
...
# ---------------------------------------------------------------------------
# Single-stock prices (from the isolated AISS store)
# ---------------------------------------------------------------------------
...
def load_stock_prices(
    ...
    """Wide (Date x ticker) adjusted-close DataFrame from the AISS price store."""
...
def load_benchmark_prices(
    ...
    """SOXX / SMH / SPY adjusted-close prices for benchmark comparison."""
...
def load_all(
    ...
    columns (SOXX/SMH/SPY).  The macro frame is FRED daily data.
    """
    ...
    _raw_cache = data_cfg.get("cache_dir", "../../price_data/semi_strategy/cache")
...
def data_quality_report(prices: pd.DataFrame, macro: pd.DataFrame) -> str:
    lines = ["=" * 64, "AISS DATA QUALITY REPORT", "=" * 64]
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO,
                        ...
                        datefmt="%H:%M:%S")
    import argparse
    ap = argparse.ArgumentParser(description="AISS data loader")

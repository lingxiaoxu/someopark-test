...
AISSStrategyRuns.py through the backtest engine and
...
# Run all 55 sets (default output to backtest_results/)
  conda run -n qlib_run --no-capture-output \\
    python qlib-main/semiconductor_strategy/AISSBatchRun.py

# Run specific named sets
  python AISSBatchRun.py --sets default momentum_heavy tech_bull_2023

# Run one thematic group only (A / B / C … L)
  python AISSBatchRun.py --group L

# Run multiple groups
  python AISSBatchRun.py --group A B L

# Custom output directory
  python AISSBatchRun.py --output-dir my_results/

# Sort output table by a different metric (default: sharpe)
  python AISSBatchRun.py --sort-by calmar

Output files
------------
  backtest_results/
    aiss_batch_summary_<timestamp>.csv        # machine-readable raw results
    aiss_batch_summary_<timestamp>.xlsx       # Excel with conditional formatting
    aiss_batch_equity_<timestamp>.csv         # daily equity curves (wide format)
"""
...
for _noisy in [
    "semiconductor_strategy.signals.composite",
    "semiconductor_strategy.signals.regime",
    "semiconductor_strategy.signals.momentum",
    "semiconductor_strategy.signals.supply_chain",
...
def _print_summary(df: pd.DataFrame, sort_by: str = "sharpe", top_n: int = 55) -> None:
    ...
    print(f"\n{'═'*110}")
    print(f"  AISS BATCH RESULTS — sorted by {sort_by} "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch backtest runner for AISSStrategyRuns.py"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "and write selected_param_set.json to semiconductor_strategy/ — this file is "
            "automatically read by AISSdailySignal on every daily/weekly/monthly run."
        ),
    )
    parser.add_argument(
    ...
    csv_path   = out_dir / f"aiss_batch_summary_{ts}.csv"
    excel_path = out_dir / f"aiss_batch_summary_{ts}.xlsx"
    equity_path = out_dir / f"aiss_batch_equity_{ts}.csv"
    ...
    if (args.save_equity and not ok.empty and (not args.select or args.tearsheet)
        ...
        try:
            ...
            print(f"  → {_n_excel} portfolio Excel files in historical_runs/semiconductor_strategy/")
    ...
    if args.select and not ok.empty:
        ...
        try:
            from semiconductor_strategy.walk_forward import (
        ...
        if _best_ps is None:
            ...
            if _oos_filter_applied:
                try:
                    from semiconductor_strategy.macro_clusters import build as _mc_build
            ...
            try:
                ...
                if _best_eq is not None and not args.no_prod_write:
                    ...
                    _best_bt = AISSBacktest(_best_cfg)
                ...
                if args.tearsheet:
                    ...
                    for _, _row in ok.iterrows():
                        ...
                        try:
                            ...
                            _pr = AISSBacktest(_pc).run(prices=prices, macro=macro)
                    ...
                    try:
                        from semiconductor_strategy.report.tearsheet import generate_tearsheet
    ...
    if args.oos_validate:
        ...
        print(f"{'═'*60}\n")
        from semiconductor_strategy.walk_forward import run_dual_mode

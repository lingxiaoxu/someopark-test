...
4. someopark correlation    — Correlation of AISS returns with pairs PnL series
...
def load_someopark_pnl(
    ...
    This is used to compute correlation between AISS and
...
def someopark_correlation_analysis(
    semiconductor_strategy_returns: pd.Series,
    someopark_returns: Optional[pd.Series] = None,
    strategy_performance_path: Optional[str] = None,
) -> Dict:
    """
    Compute correlation between AISS and someopark pairs trading.
    ...
    aligned = pd.concat([
        semiconductor_strategy_returns.rename("semiconductor_strategy"),
    ...
    full_corr = float(aligned.corr().loc["semiconductor_strategy", "someopark"])

    # Rolling 63-day correlation
    rolling_corr = (
        aligned["semiconductor_strategy"]
    ...
    w = 0.5  # Equal weight
    combined = w * aligned["semiconductor_strategy"] + (1 - w) * aligned["someopark"]
    vol_combined = float(combined.std() * np.sqrt(252))
    vol_sr = float(aligned["semiconductor_strategy"].std() * np.sqrt(252))
    ...
    sr_sharpe = sharpe_ratio(aligned["semiconductor_strategy"])
    ...
    return {
        ...
        "semiconductor_strategy_sharpe": round(sr_sharpe, 4),

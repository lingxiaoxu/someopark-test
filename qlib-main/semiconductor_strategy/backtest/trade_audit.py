...
用法（在 someopark-test/ 目录下）：
    ...
    python qlib-main/semiconductor_strategy/backtest/trade_audit.py [--output-dir DIR]

2026-07-21: 全体包进 main() + __main__ 守卫——此前模块顶层执行,import 即跑全量
审计并覆写 historical_runs/ 的 csv(import 副作用地雷)。新增 --output-dir
(默认仍是 historical_runs/semiconductor_strategy/,无参调用行为不变)。
"""
...
SR_ROOT   = os.path.join(REPO_ROOT, "qlib-main", "semiconductor_strategy")


_AMP_CACHE: dict = {}   # 通路③:需求周期序列/图谱敏感度按进程缓存一次
...
def main(output_dir: str | None = None) -> None:
    ...
    from semiconductor_strategy.backtest.engine    import AISSBacktest  # noqa: F401
    ...
    from semiconductor_strategy.data.loader         import load_prices, load_macro_data as load_macro
    ...
    from semiconductor_strategy.data.loader import load_returns
    ...
    for dt in all_dates:
        ...
        if is_scheduled or is_emergency:
            ...
            if _amp_cfg.get("enabled", False):
                if "_amp_series" not in _AMP_CACHE:
                    try:
                        ...
                        from semiconductor_strategy.data.loader import load_config as _lc
                        from semiconductor_strategy.signals.supply_chain import (demand_sensitivity,
                                                                                 load_graph_from_config)
                        _AMP_CACHE["_amp_series"] = _alt.load_ai_demand_cycle()
                        _AMP_CACHE["_amp_sens"] = (demand_sensitivity(load_graph_from_config(
            ...
            adj_weights, cash_pct, flags = apply_risk_controls(
                ...
                # 日收益累乘重建净值,断路器/离底释放在审计重放中同语义生效
                equity_curve       = ((1.0 + portfolio_daily_returns).cumprod()
                                      if len(portfolio_daily_returns) > 0 else None),
    ...
    SECTORS = ETF_TICKERS
    SECTOR_LABEL = {
        "ai_gpu":"GPU","custom_asic":"ASIC","equipment":"Equip","memory_hbm":"Mem",
        "foundry":"Fdry","analog_defense":"Anlg","logic_cpu":"CPU","rf_edge":"RF",
    }
    ...
    # ── 2. Save to CSV ────────────────────────────────────────────────────────
    _out_dir = output_dir or os.path.join(REPO_ROOT, "historical_runs", "semiconductor_strategy")
    out_path = os.path.join(_out_dir, "trade_audit.xlsx")
...
if __name__ == "__main__":
    import argparse
    _ap = argparse.ArgumentParser(description="AISS trade audit replay")
    _ap.add_argument("--output-dir", default=None,
                     help="CSV output dir (default: historical_runs/semiconductor_strategy/)")

...
Backtest Engine
===============
Event-driven monthly backtest for the AISS strategy.
...
Primary path uses qlib's full backtest infrastructure:
    ...
    - AISSWeightStrategy (qlib WeightStrategyBase): signal→weight logic
...
class BacktestResult:
    ...
    def summary(self) -> str:
        ...
        lines = [
            "=" * 60,
            "AISS (AI Infra & Semiconductor) BACKTEST SUMMARY",
            "=" * 60,
...
class AISSBacktest:
    ...
    Usage:
        bt = AISSBacktest(config)
        result = bt.run()
        print(result.summary())
    """

    def __init__(self, config: dict, mlflow_experiment: str = "semiconductor_strategy_backtest"):
        self.cfg = config
    ...
    def _run_qlib(
        ...
        Full qlib infrastructure backtest:
            ...
            AISSWeightStrategy + SectorSimulatorExecutor + backtest_loop
        """
        from .qlib_adapter import SectorETFExchange, SectorSimulatorExecutor
        from ..portfolio.strategy import AISSWeightStrategy
    ...
    def _record_experiment(
        ...
        try:
            ...
            # AISS keeps mlruns INSIDE the strategy dir (never the shared qlib-main/mlruns).
            mlruns_path = Path(__file__).parent.parent.resolve() / "mlruns"
            mlruns_path.mkdir(parents=True, exist_ok=True)
            uri = f"sqlite:///{mlruns_path}/mlflow.db"
            exp_manager = MLflowExpManager(uri=uri, default_exp_name="aiss_backtest")
            ...
            with recorder_client.start(experiment_name=self._mlflow_experiment):
                ...
                recorder_client.set_tags(
                    bt_start=bt_start,
                    bt_end=bt_end,
                    strategy="semiconductor_strategy",
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run AISS backtest")
    ...
    # active_cfg: used for AISSBacktest (strategy keys — overridden by param set)
    ...
    if args.walk_forward:
        from semiconductor_strategy.walk_forward import WalkForwardAnalyzer, run_dual_mode
        wf_kwargs = dict(
        ...
        engine = AISSBacktest(active_cfg)

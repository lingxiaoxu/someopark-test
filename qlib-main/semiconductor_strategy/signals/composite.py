"""
composite.py — AISS 4-factor composite signal
=============================================
Blends the four AISS factors into a single cross-sectional score per subsector
...
The function signature mirrors the AISS ``compute_composite_signals`` so the
copied engine calls it unchanged; the legacy value/new-signal parameters are
accepted but ignored (AISS V1 has no P/E value factor).  Returns the same
``(composite_df, regime_monthly, components)`` contract.

Regime conditioning reuses the AISS 4-state machine (risk_on / transition_up /
transition_down / risk_off) with AISS-tightened VIX thresholds (22/28) and
AISS regime-weight multipliers; a defensive bonus is applied to the
...
# --- AISS 4-factor weights (must sum to 1.0) -------------------------------
DEFAULT_WEIGHTS = {
    "cs_momentum":  0.30,
    "supply_chain": 0.35,
    "capex_pulse":  0.25,
...
# --- AISS regime-conditional weight multipliers (4-state machine) ----------
...
DEFENSIVE_TICKERS = ["analog_defense"]
DEFENSIVE_BONUS_RISK_OFF = 0.40

# Per-subsector sensitivity to the AI-CapEx pulse (capex factor beta).
CAPEX_BETA = {
    "ai_gpu": 1.0, "custom_asic": 0.8, "equipment": 0.6, "memory_hbm": 0.5,
    "foundry": 0.5, "logic_cpu": 0.3, "rf_edge": -0.2, "analog_defense": -0.5,
}
...
def _validate_weights(weights: Dict[str, float]) -> None:
    ...
    if abs(total - 1.0) > 1e-6:
        logger.warning("AISS signal weights sum to %.4f (expected 1.0): %s", total, core)
...
def _load_aux_signals(prices: pd.DataFrame, signal_kwargs: dict) -> dict:
    ...
    try:
        ...
        from pathlib import Path
        sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
        from semiconductor_strategy.data import company_signals as comp  # type: ignore
        from semiconductor_strategy.data import industry_signals as ind  # type: ignore
    try:
        out["capex"] = comp.load_capex_pulse(end=end)
...
def _build_cycle_regime_tilt(
    ...
    high_beta = {"ai_gpu", "logic_cpu", "memory_hbm"}
...
def compute_composite_signals(
    ...
    value_source: Optional[str] = None,       # accepted & ignored (AISS V1 has no value factor)
    ...
    benchmark_prices: Optional[pd.Series] = None,  # ignored (AISS uses dedicated benchmarks)
) -> Tuple[pd.DataFrame, pd.Series, Dict[str, pd.DataFrame]]:
    """Compute regime-conditioned AISS composite z-scores per subsector.
    ...
    components = {
        "cs_momentum": cs_mom,
        "cs_mom": cs_mom,                      # alias for any AISS-era reader
    ...
    logger.info("AISS composite: %d months × %d subsectors (last regime=%s)",
...
def get_current_signals(config: Optional[dict] = None, as_of: Optional[str] = None) -> dict:
    """Return the latest-month AISS composite + per-factor breakdown (for live use)."""
    try:
        ...
        from pathlib import Path
        sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
        from semiconductor_strategy.data.loader import load_all, load_config  # type: ignore

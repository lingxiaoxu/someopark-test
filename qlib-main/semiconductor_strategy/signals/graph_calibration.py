"""
graph_calibration.py — empirical lead-lag calibration of the AISS supply-chain graph
=====================================================================================
...
semiconductor beta (all subsectors co-move).  This module calibrates each edge's
...
Usage:
    conda run -n qlib_run python -m semiconductor_strategy.signals.graph_calibration
"""
...
from semiconductor_strategy.signals.supply_chain import (  # noqa: E402
    SUPPLY_CHAIN_GRAPH, NODE_AI_CAPEX, NODE_CONSUMER, NODE_PMI, SPECIAL_NODES,
)
...
# Candidate inbound edges for logic_cpu (V1 had NONE → its supply_chain score was
# always 0).  Economically motivated; calibration picks the lag and the keep gate
# (|IC| ≥ KEEP_IC_THRESHOLD) decides which survive.
CANDIDATE_EDGES: List[Tuple[str, str, float, str]] = [
    (NODE_CONSUMER,  "logic_cpu", 0.60, "PC/client CPU demand (Intel/AMD client)"),
    ("foundry",      "logic_cpu", 0.50, "AMD/ARM are TSMC fabless customers; capacity leads"),
    (NODE_AI_CAPEX,  "logic_cpu", 0.40, "server-CPU attach to datacenter buildout"),
]
...
def _load_inputs() -> dict:
    ...
    # residualize: remove the common semiconductor factor (cross-sectional mean)
    ...
    return {"resid": resid, "etfs": etfs, "idx": idx, "capex_m": capex_m, "pmi_m": pmi_m}
...
def _source_series(node: str, data: dict) -> Optional[pd.Series]:
    ...
    if node == NODE_CONSUMER:
        return resid["rf_edge"] if "rf_edge" in resid else None
    if node == NODE_PMI:
        return data["pmi_m"]
...
def _print_report(report: dict) -> None:
    print("=" * 96)
    print(f"AISS SUPPLY-CHAIN GRAPH — empirical lag calibration (residual returns)")
...
def main() -> None:
    ...
    out_dir = _QLIB_DIR / "semiconductor_strategy" / "backtest_results"

...
strategy.py
===========
AISSWeightStrategy — adapts our AISS logic into qlib's
...
if not _QLIB_STRATEGY_AVAILABLE:
    ...
    class AISSWeightStrategy:  # type: ignore
        def __init__(self, *a, **kw):
            raise RuntimeError("qlib strategy infrastructure not available")
else:
    class AISSWeightStrategy(WeightStrategyBase):
        """
        Monthly AISS strategy integrated into qlib's WeightStrategyBase.
        ...
        def __init__(
            ...
            **kwargs,
            ...
            # qlib 路径是 batch/WF 的真实生产路径(engine._run_native 沙盒实测不被调用),
            # 与 engine 原生循环 / AISSdailySignal 共用纯函数 compute_exposure_amplifier。
            ...
            if self._amp_on:
                try:
                    ...
                    self._amp_series = _alt.load_ai_demand_cycle()
                    if self._amp_cfg.get("graph_weighted", True):
                        self._amp_sens = demand_sensitivity(load_graph_from_config(
            ...
            # 都是 true,但只有 _run_native 读它们;qlib 路径(本策略的生产实际
        ...
        def generate_trade_decision(self, execute_result=None) -> "TradeDecisionWO":
            ...
            # ── C5.b: VIX 紧急调仓的滞回冷却(镜像 _run_native)────────────
            ...
            adj_weights, cash_pct, flags = apply_risk_controls(
                ...
                exposure_note=(f"demand_z={_z:+.2f} phi={_phi:.2f}" if self._amp_on else ""),
            )
            if self._amp_on:
                flags.demand_z = _z
        ...
        def _exposure_mult(self, dt: "pd.Timestamp", weights: "pd.Series"):
            """PIT: 需求周期取 ≤dt 的最后一个可得值;缺数 → (1.0, nan, nan)。"""

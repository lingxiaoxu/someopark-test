"""
validate.py — AISS win-criterion validation
============================================
THE go/no-go gate for the AI Infra & Semiconductor Strategy.

The strategy exists only to be better than simply holding SOXX or SMH, so this
module backtests AISS (a chosen param set, default = production default) over the
same window as SOXX / SMH / SPY buy-and-hold and reports CAGR / Vol / Sharpe /
Calmar / MaxDD for all four, plus the explicit verdict:

    PASS  iff  AISS beats BOTH SOXX and SMH on Sharpe AND CAGR.

Usage:
    python -m semiconductor_strategy.validate                 # default param set
    python -m semiconductor_strategy.validate --param-set momentum_heavy
    python -m semiconductor_strategy.validate --json out.json
"""
...
log = logging.getLogger("aiss.validate")
...
def validate(param_set: str = "default", config: dict | None = None,
    ...
    a = out["AISS"]
    hurdles = [b for b in ("SOXX", "SMH") if b in out]
    beats = {
...
def _print(report: dict) -> None:
    print("=" * 78)
    print(f"AISS WIN-CRITERION VALIDATION — param_set={report['param_set']}  "
    ...
    print("=" * 78)
    print(f"WIN CRITERION (beat SOXX & SMH on Sharpe AND CAGR): "
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="AISS win-criterion validation")

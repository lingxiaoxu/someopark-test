...
def _add_title_page(pdf: PdfPages, metrics: dict, config: dict,
    ...
    ax.text(0.040, 0.938, "AI Infra & Semiconductor Strategy  —  Performance Tearsheet",
            transform=ax.transAxes, ha="left", va="center",
    ...
    # ── FOOTER ────────────────────────────────────────────────────────────────
    ax.text(0.5, 0.005,
            "Someo Park  ·  AI Infra & Semiconductor Strategy v1.0  ·  "
...
def _add_benchmark_comparison_page(pdf, result, prices) -> None:
    """AISS vs SOXX / SMH / SPY — normalized equity overlay + win-criterion table.

    SOXX & SMH are the strategy's mandate hurdle; this page makes the
    "beat both on Sharpe AND CAGR" verdict visible in the PDF.
    """
    if prices is None:
        return
    ...
    curves = {"AISS": eq / eq.iloc[0]}
    rets = {"AISS": eq.pct_change()}
    colors = {"AISS": PALETTE["strategy"], "SOXX": "#d62728", "SMH": "#ff7f0e", "SPY": PALETTE.get("benchmark", "#888")}
    for b in ("SOXX", "SMH", "SPY"):
        if b in prices.columns:
            s = prices[b].loc[start:end].dropna()
            if len(s) > 1:
                curves[b] = s / s.iloc[0]
                rets[b] = s.pct_change()
    ...
    fig = plt.figure(figsize=(14, 9))
    fig.patch.set_facecolor(PALETTE["bg"])
    fig.suptitle("AISS vs Semiconductor Benchmarks (SOXX / SMH)", fontsize=13,
                 fontweight="bold", color=PALETTE["header_mid"], y=0.97)
    ax = fig.add_axes([0.07, 0.34, 0.88, 0.55])
    ax.set_facecolor(PALETTE["surface"])
    for name, c in curves.items():
        ax.plot(c.index, c.values, label=name, linewidth=2.0 if name == "AISS" else 1.2,
                color=colors.get(name), alpha=1.0 if name == "AISS" else 0.8)
    ax.set_yscale("log")
    ax.set_ylabel("Growth of $1 (log)", fontsize=9)
    ax.legend(fontsize=9, loc="upper left")
    ax.grid(color=PALETTE["grid"], alpha=0.7, linewidth=0.5)
    ...
    # metrics table
    rows, cell = [], []
    a_cagr, a_sh, a_dd = _met(rets["AISS"])
    for name in ("AISS", "SOXX", "SMH", "SPY"):
        if name not in rets:
            continue
        cg, sh, dd = _met(rets[name])
        rows.append(name)
        cell.append([f"{cg:.1%}", f"{sh:.2f}", f"{dd:.1%}"])
    ax2 = fig.add_axes([0.07, 0.08, 0.55, 0.20]); ax2.axis("off")
    tbl = ax2.table(cellText=cell, rowLabels=rows,
                    colLabels=["CAGR", "Sharpe", "MaxDD"], loc="center", cellLoc="center")
    tbl.auto_set_font_size(False); tbl.set_fontsize(10); tbl.scale(1, 1.5)

    beats_soxx = ("SOXX" in rets) and (a_sh > _met(rets["SOXX"])[1]) and (a_cagr > _met(rets["SOXX"])[0])
    beats_smh = ("SMH" in rets) and (a_sh > _met(rets["SMH"])[1]) and (a_cagr > _met(rets["SMH"])[0])
    verdict = "PASS — AISS beats SOXX & SMH on Sharpe AND CAGR" if (beats_soxx and beats_smh) \
        else "FAIL — does not beat both benchmarks on both metrics"
    fig.text(0.66, 0.18, "WIN CRITERION", fontsize=11, fontweight="bold", color=PALETTE["header_mid"])
    fig.text(0.66, 0.13, verdict, fontsize=10,
             color=(PALETTE.get("positive", "green") if (beats_soxx and beats_smh) else PALETTE.get("negative", "red")),
             wrap=True)
    pdf.savefig(fig, bbox_inches="tight")
    plt.close(fig)
...
def generate_tearsheet(
    ...
    Parameters
    ----------
    result            : BacktestResult from AISSBacktest.run()
    output_path       : Full PDF path (auto-generated if None)
    prices            : Raw prices DataFrame (for correlation page)
    daily_report_dir  : Directory containing aiss_daily_report_*.json files
                        (defaults to <repo>/trading_signals/)
    batch_df          : DataFrame from AISSBatchRun (all 59 param sets)
    ...
    for d in search_dirs:
        if d.exists():
            candidates = sorted(d.glob("aiss_daily_report_*.json"), key=os.path.getmtime)
            if candidates:
    ...
    with PdfPages(str(output_path)) as pdf:
        ...
        # ── P2b: AISS vs SOXX / SMH / SPY (win-criterion overlay) ────────────
        _add_benchmark_comparison_page(pdf, result, prices)
        ...
        # PDF metadata
        d = pdf.infodict()
        d["Title"]        = "AI Infra & Semiconductor Strategy — Performance Tearsheet"
        d["Author"]       = "Someo Park"
        d["Subject"]      = "Institutional Sector ETF Rotation Backtest"
        d["Keywords"]     = "AISS, ETF, risk parity, momentum, backtest"
...
def compute_mcps_rolling_equity(
    ...
    for name in set_names:
        try:
            ...
            res = AISSBacktest(cfg).run(prices=prices, macro=macro)
...
def main() -> None:
    """CLI: run an AISS backtest and emit a PDF tearsheet (vs SOXX/SMH/SPY)."""
    import argparse
    import sys
    from pathlib import Path as _P
    logging.basicConfig(level=logging.ERROR, format="%(asctime)s %(levelname)s: %(message)s")
    _qlib = _P(__file__).resolve().parents[2]
    if str(_qlib) not in sys.path:
        sys.path.insert(0, str(_qlib))
    from semiconductor_strategy.data.loader import load_config, load_all
    from semiconductor_strategy.backtest.engine import AISSBacktest
    from semiconductor_strategy.AISSStrategyRuns import PARAM_SETS, apply_param_set

    ap = argparse.ArgumentParser(description="AISS PDF tearsheet")
    ...
    run_cfg = apply_param_set(cfg, PARAM_SETS.get(args.param_set, {}))
    result = AISSBacktest(run_cfg).run(prices, macro)
    out = generate_tearsheet(result, output_path=args.output, prices=prices,

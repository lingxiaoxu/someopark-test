#!/bin/bash
# =============================================================================
# aeus_pipeline.sh — AEUS (AI Infra & Electric Utilities Strategy) pipeline
# =============================================================================
# Single production entrypoint for the AEUS strategy.  Mirrors SSRS's pipeline
# but is purpose-built for AEUS: individual-stock data, no EPS/value modes.
#
...
#     daily         Generate today's AEUS signal + update inventory  (NYSE holiday-aware)
...
#     validate      Backtest + compare vs XLU/GRID/SPY (win criterion)
...
#     tearsheet     Full PDF tearsheet (vs XLU/GRID/SPY)
...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"     # .../electric_utilities_strategy
...
# AEUS handles --skip-holiday in the shell (the NYSE check is a shell concern)
...
cd "$QLIB_DIR"   # so `python -m electric_utilities_strategy.*` resolves
...
run_update_data() {
    hr; log "AEUS update_data — incremental refresh"; hr
    local rc=0 n=0
    _step() {   # _step "<描述>" <cmd...>
    ...
    # AEUS_PLAN §4.1: 9 步(AISS 7 步的电力版扩展;每步独立失败不阻断)
    _step "prices (Polygon incremental)"          -m $PKG.data.aeus_fetch_prices  --update "$@"
    _step "CapEx pulse (yfinance, 与 AISS 共源)"   -m $PKG.data.company_signals   --update-capex "$@"
    _step "utility+water CapEx (SEC XBRL)"        -m $PKG.data.company_signals   --update-utility-capex --update-water-capex "$@"
    _step "hyperscaler actual CapEx (SEC XBRL)"   -m $PKG.data.company_signals   --update-hyperscaler-capex "$@"
    _step "EIA monthly (售电/燃料/backlog/gas/IPUTIL)" -m $PKG.data.industry_signals --update-elec-gen --update-fuel-mix --update-backlog --update-gas --update-pmi "$@"
    _step "EIA daily demand + STEO 度日"           -m $PKG.data.altdata_signals   --update-demand --update-dd "$@"
    _step "EIA capacity + 分州电价 + FRED altdata"  -m $PKG.data.altdata_signals   --update-capacity --update-state-price --update-fred "$@"
    _step "ERCOT (DAM SPP + AS, 增量)"             -m $PKG.data.ercot_signals     --update "$@"
    _step "PJM (门控;未接线时单行日志跳过)"          -m $PKG.data.pjm_signals       --update "$@"
    if [ "$rc" -eq 0 ]; then log "update_data complete (9/9 ok)."
...
case "$MODE" in
    update_data|update-data)
        run_update_data "$@" 2>&1 | tee "$LOG_DIR/aeus_update_data_$TS.log"
        ;;

    daily)
        check_nyse_open
        # Event-risk shared data refresh (NON-FATAL, idempotent): keeps the event_risk
        # price store + NFP/bellwether calendars current so the AEUS event-risk overlay
        # (in AEUSdailySignal) reads fresh data even if the someopark conductor hasn't
        # run its refresh yet.  Safe to run from both pipelines (dedupe keep-last).
        # AEUS_PLAN §5.1/§10-6: RefreshEventRiskData.py 刷新的是**半导体**事件
        # 风险 store(semiconductor_universe.json 域)。AEUS phase-1 event_derisk
        # 已在 config 关闭,此步注释掉以免去刷别家数据;C 级接线(电力事件
        # universe + 检测器参数化)获批后恢复为电力版调用。
        # hr; log "event-risk data refresh (non-fatal)"; hr
        # PY "$REPO_ROOT/RefreshEventRiskData.py" 2>&1 | tee "$LOG_DIR/aeus_event_refresh_$TS.log" \
        #     || log "WARN: event-risk data refresh failed (non-fatal, continuing)"
        log "event-risk refresh SKIPPED (phase-1: config event_derisk.enabled=false; TODO C-level wiring)"
        ...
        # 与 macro self-heal (AEUSdailySignal._load_macro_from_store) 同模式:
        # 补一次、失败降级、**绝不阻断 signal** —— 数据旧总好过当天没信号。
        # 不传 "$@": daily 的参数是 AEUSdailySignal 的(--date/--dry-run 等),
        ...
        # AEUSdailySignal refreshes its own price store (incremental, before marking)
        # so positions mark to today's close regardless of how it's invoked.
        hr; log "AEUS daily signal"; hr
        PY -m $PKG.AEUSdailySignal "$@" 2>&1 | tee "$LOG_DIR/aeus_daily_$TS.log"
        ;;

    dry-run|dryrun)
        PY -m $PKG.AEUSdailySignal --dry-run "$@" 2>&1 | tee "$LOG_DIR/aeus_dryrun_$TS.log"
        ;;

    backtest)
        hr; log "AEUS single backtest"; hr
        PY -m $PKG.backtest.engine "$@" 2>&1 | tee "$LOG_DIR/aeus_backtest_$TS.log"
        ;;

    batch)
        hr; log "AEUS batch (all param sets)"; hr
        PY -m $PKG.AEUSBatchRun "$@" 2>&1 | tee "$LOG_DIR/aeus_batch_$TS.log"
        ;;

    select)
        hr; log "AEUS production param selection (batch + WF OOS + MCPS)"; hr
        PY -m $PKG.AEUSBatchRun --select --save-equity "$@" 2>&1 | tee "$LOG_DIR/aeus_select_$TS.log"
        ;;

    walk-forward|wf)
        hr; log "AEUS walk-forward IS/OOS"; hr
        PY -m $PKG.walk_forward "$@" 2>&1 | tee "$LOG_DIR/aeus_wf_$TS.log"
        ;;

    daily_backtest|daily-backtest)
        hr; log "AEUS V1+V2 backtest/selection suite"; hr
        bash "$SCRIPT_DIR/daily_backtest.sh" "$@"
        ;;

    validate)
        hr; log "AEUS validation vs XLU / GRID / SPY (win criterion)"; hr
        PY -m $PKG.validate "$@" 2>&1 | tee "$LOG_DIR/aeus_validate_$TS.log"
        ;;

    tearsheet)
        hr; log "AEUS tearsheet (PDF, vs XLU/GRID/SPY)"; hr
        PY -m $PKG.report.tearsheet "$@" 2>&1 | tee "$LOG_DIR/aeus_tearsheet_$TS.log"
        ;;

    test)
        hr; log "AEUS test suite"; hr
        PY -m pytest "$SCRIPT_DIR/tests/" -v --tb=short "$@"
        ;;

    status)
        PY -m $PKG.AEUSdailySignal --dry-run --status-only 2>/dev/null || \
          PY -c "import json,glob,os; \
d=json.load(open('$SCRIPT_DIR/inventory_aeus.json')) if os.path.exists('$SCRIPT_DIR/inventory_aeus.json') else {}; \
print('inventory as_of:', d.get('as_of')); print('holdings:', d.get('holdings'))"
    ...
    weekly)
        ...
        # (AEUS has no EPS step; SSRS's EPS refresh is replaced by data/PIT health checks.)
        WK_LOG="$LOG_DIR/aeus_weekly_$TS.log"
        hr; log "AEUS WEEKLY maintenance (price refresh + data/PIT health + weekly review + dry-run)"; hr
        # Incremental price refresh for the FULL universe incl. benchmarks (XLU/GRID/SPY)
        # BEFORE verifying. Benchmarks are not in the daily signal's load path
        # (daily loads SR ETFs + SPY + the stock universe; XLU/GRID only refresh when
        ...
        log "0/4 price refresh (incremental, full universe incl. XLU/GRID benchmarks)…"
        PY -m $PKG.data.aeus_fetch_prices --update 2>&1 | tee -a "$WK_LOG" \
          || log "  WARN: weekly price refresh failed (non-fatal, verify will report)"
        log "1/4 data + PIT health checks (prices / company / industry / altdata / ercot / pjm coverage)…"
        ...
        # (aeus-weekly, 周日 03:30 ET),它的 runbook 判"干净成功"的条件是
        ...
        VERIFY_OUT="/tmp/aeus_weekly_verify_$TS.txt"   # 收尾横幅要靠它列出具体是哪几条
        PIT_STALE=0
        PY -m $PKG.data.aeus_fetch_prices --verify 2>&1 | tee -a "$WK_LOG" "$VERIFY_OUT" || PIT_STALE=1
        ...
        # 2026-08-31: AEUS 比 AISS 多三个**活的**数据层(altdata/ercot 喂图谱节点、
        # shortage exposure amplifier 与 ERCOT external tilt;AISS 的 altdata_signals
        # 是零引用死代码,三行 verify 恰好够)。机械镜像 AISS 的三行结构会让这些层
        # 长期陈旧时 weekly 照报 clean success —— 逐层补齐体检。
        # pjm --verify 在 gate 关闭(config pjm.enabled:false)时返回 OK 不误报,
        # 接线后空/陈旧才退非零,故无需条件分支。
        PY -m $PKG.data.altdata_signals  --verify 2>&1 | tee -a "$WK_LOG" "$VERIFY_OUT" || PIT_STALE=1
        PY -m $PKG.data.ercot_signals    --verify 2>&1 | tee -a "$WK_LOG" "$VERIFY_OUT" || PIT_STALE=1
        PY -m $PKG.data.pjm_signals      --verify 2>&1 | tee -a "$WK_LOG" "$VERIFY_OUT" || PIT_STALE=1
        if [ "$PIT_STALE" -ne 0 ]; then
            hr | tee -a "$WK_LOG"
            log "!!! PIT DATA HEALTH FAILED — 上面标了 ← STALE 的序列已过期。它们喂 composite" | tee -a "$WK_LOG"
            log "!!! 的 0.70 权重(capex_tilt .25 + cycle_regime .10 + supply_chain .35),以及" | tee -a "$WK_LOG"
            log "!!! altdata/ercot 层的图谱节点 + shortage amplifier + external tilts。" | tee -a "$WK_LOG"
        ...
        PY -m $PKG.AEUSdailySignal --dry-run 2>&1 | tee -a "$WK_LOG"
    ...
    monthly)
        ...
        # AEUS refreshes P0 via daily_backtest (V1+V2 select → restore V1), which
        # encompasses the per-version 'select'; then a force-rebalance daily signal.
        check_nyse_open
        MO_LOG="$LOG_DIR/aeus_monthly_$TS.log"
        hr; log "AEUS MONTHLY (V1+V2 select suite + restore V1 + force-rebalance)"; hr
        ...
        PY -m $PKG.AEUSdailySignal --force-rebalance "$@" 2>&1 | tee -a "$MO_LOG"
    ...
    *)
        echo "Unknown mode: $MODE"; echo "Run: bash aeus_pipeline.sh help"; exit 2 ;;
esac

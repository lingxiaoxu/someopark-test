...
# Mirror SSRS: Excel outputs go to repo-root historical_runs/electric_utilities_strategy/
# (parallel to sector_rotation's historical_runs/sector_rotation/).
_HIST_DIR = _THIS_DIR.parent.parent / "historical_runs" / "electric_utilities_strategy"
...
class SectorRotationRecord:
    ...
    def export_portfolio_excel(self, output_path: Path = None,
        ...
        if output_path is None:
            ...
            output_path = _HIST_DIR / f"aeus_portfolio_{self.param_set}_{ver}_{span}_{mode}_{ts}.xlsx"
    ...
    def _write_stock_decomp_sheets(self, wb) -> None:
        ...
        from electric_utilities_strategy.data.aeus_fetch_prices import load_prices_wide, first_available_dates
    ...
    def export_monitor_excel(
        ...
        if output_path is None:
            ...
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            ver = self.signal_version
            output_path = _HIST_DIR / f"monitor_aeus_{self.param_set}_{ver}_daily_{ts}.xlsx"
...
def export_wf_diagnostic_excel(
    ...
    if output_path is None:
        ...
        output_path = _HIST_DIR / f"wf_diagnostic_aeus_{signal_version}_IS-OOS_{wf_mode}_{mode}_{ts}.xlsx"

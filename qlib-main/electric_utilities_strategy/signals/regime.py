...
def _classify_regime_row(
    ...
    vix_high: float = 22.0,        # AEUS: tighter than AEUS 25 (Beta ~2.0)
    vix_extreme: float = 28.0,     # AEUS: tighter than AEUS 35
...
def compute_regime_rules(
    macro: pd.DataFrame,
    vix_high_threshold: float = 22.0,        # AEUS default (AEUS was 25)
    vix_extreme_threshold: float = 28.0,     # AEUS default (AEUS was 35)
    hy_spread_high_bps: float = 450.0,
    yield_curve_inversion: float = -0.10,
    ism_expansion: float = 50.0,
    smoothing_days: int = 5,
    **_ignored,   # tolerate AEUS-specific regime kwargs (capex_strong/weak_zscore)
) -> pd.Series:
    """
...
if __name__ == "__main__":
    ...
    from electric_utilities_strategy.data.loader import load_all

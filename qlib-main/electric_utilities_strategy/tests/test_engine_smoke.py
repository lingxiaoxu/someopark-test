...
Exercises the complete pipeline:
    ...
    → AEUSWeightStrategy (WeightStrategyBase)
...
from electric_utilities_strategy.backtest.engine import AEUSBacktest, BacktestResult
...
_TICKERS = ["nuclear_fuel", "gas_midstream", "grid_equipment", "grid_epc",
            "ipp_wholesale", "regulated_mega", "regional_utility",
            "dc_power_cooling", "renewables_storage", "water_cooling"]  # AEUS subsector assets
_BENCH = "SPY"
_START = "2020-01-01"
_END   = "2021-06-30"
...
def _minimal_config() -> dict:
    ...
    return {
        ...
        "signals": {
            ...
            "cs_momentum": {"lookback_months": 12, "skip_months": 1, "zscore_window": 36},
            "supply_chain": {"use_external_macro": False},  # offline: price-proxy only
            "regime": {"method": "rules"},
        },
        "portfolio": {
            "optimizer":    "inv_vol",
...
class TestEngineSmokeNative(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        from electric_utilities_strategy.backtest import engine as _eng
        ...
        bt = AEUSBacktest(cfg, mlflow_experiment="electric_utilities_strategy_test")
...
class TestBacktestResultFields(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        prices = _make_prices()
        macro  = _make_macro()
        bt = AEUSBacktest(_minimal_config(), mlflow_experiment="electric_utilities_strategy_test")
        cls.result = bt.run(prices=prices, macro=macro)
    ...
    def test_summary_runs_without_error(self):
        ...
        self.assertIn("AEUS (AI Infra & Electric Utilities) BACKTEST SUMMARY", s)

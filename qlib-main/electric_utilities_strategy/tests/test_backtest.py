...
from electric_utilities_strategy.backtest.costs import (
...
from electric_utilities_strategy.backtest.metrics import (
    annualized_return,
    annualized_vol,
    sharpe_ratio,
    max_drawdown,
...
class TestTransactionCosts(unittest.TestCase):
    ...
    def test_cost_bps_by_tier(self):
        self.assertEqual(get_one_way_cost_bps("NEE"), 3)   # Tier 1 (mega-cap)
        self.assertEqual(get_one_way_cost_bps("BWXT"), 5)  # Tier 2 (large-cap)
        self.assertEqual(get_one_way_cost_bps("OKLO"), 8)  # Tier 3 (small/IPO)
...
class TestSubperiodAnalysis(unittest.TestCase):
    ...
    def test_empty_schema_has_core_metric_columns(self):
        # 空表 schema 必须含消费方选用的核心指标列(2026-07-26: 防"换个地方 KeyError")
        ret = _make_returns_series(n=10)
        sp = subperiod_analysis(ret)
    ...
    def test_dynamic_recent_subperiods_cover_2025_plus(self):
        # 2025 起的短窗(静态 SUBPERIODS 之外)必须产出动态子期行
...
class TestExpectedMaxSharpe(unittest.TestCase):
    ...
    def _f(self):
        from electric_utilities_strategy.walk_forward import expected_max_sharpe

"""
aeus_matrix.py — full 33×V1/V2 backtest + validate matrix (in-process).
=======================================================================
Runs EVERY param set under BOTH signal versions, twice over:
  ...
  PART 2  win-criterion validate — 33 × {v1,v2} = 66 runs; asserts no error,
          tallies how many beat XLU & GRID (PASS).
...
from electric_utilities_strategy.validate import validate as run_validate
...
def main():
    ...
    for ps in names:
        for ver in ("v1", "v2"):
            try:
                ...
                r = AEUSBacktest(cfg).run(prices, macro)
                m = r.metrics
    ...
    print("\n== PART 2: validate (win-criterion vs XLU/GRID) — 33 × V1/V2 ==")

"""
aeus_verify_excel.py — deep per-sheet verification of AEUS backtest Excel.
=========================================================================
Opens EVERY sheet of EVERY AEUS portfolio Excel and runs computation
cross-checks — not just empty/null/0/NaN, but *correctness*:
  ...
  • sector_pnl_acc    : == cumsum(sector_pnl_daily) per subsector
  • pnl reconciliation: daily_pnl total == Σ(sector_pnl_daily across subsectors)
  • stock_decomp      : each {subsector}/{stock} column-group sums to its parent
                        subsector column (weights err < 1e-3)
  ...
  • structural        : all expected sheets present, none empty/all-null,
                        equity all positive, V1/V2 rebalance cadence

NOTE on Sharpe: the engine/summary use qlib's compound-interest Sharpe
(``metrics.sharpe_ratio`` → annualized compound return / annualized vol).
This module recomputes with the SAME convention so the cross-check is
apples-to-apples (a simple arithmetic Sharpe would differ by convention, not
by error).

Usage:
    conda run -n qlib_run python -m electric_utilities_strategy.tests.aeus_verify_excel [YYYYMMDD] [--dir PATH]
...
# qlib-ci Sharpe (the convention the engine/summary use)
_HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.abspath(os.path.join(_HERE, "..", "..")))  # qlib-main/
from electric_utilities_strategy.backtest.metrics import sharpe_ratio as _ci_sharpe

DEFAULT_DIR = os.path.abspath(os.path.join(
    _HERE, "..", "..", "..", "historical_runs", "electric_utilities_strategy"))
...
EXPECTED_DECOMP = [
    "sector_prices_stock_decomp","sector_weights_stock_decomp","sector_wt_pct_stock_decomp",
    "share_hist_stock_decomp","cost_basis_stock_decomp","sector_pnl_stock_decomp",
    "sector_contrib_stock_decomp",
]
VALID_REGIMES = {
...
# Tolerances
TOL_SHARPE = 0.25     # equity-derived vs summary (series nuance: equity path vs engine daily_returns)
TOL_CAGR   = 0.02
TOL_MAXDD  = 0.02
TOL_DECOMP = 1e-3
...
def _summary_metrics(df):
    """Fuzzy-extract sharpe/cagr/maxdd from the summary sheet."""
    out = {}
    if df is None:
        return out
...
def verify_file(path):
    """Return (issues:list[str], nreb:int|None)."""
    issues = []
    wb = load_workbook(path, read_only=True, data_only=True)
    sheets = set(wb.sheetnames)

    # structural — expected sheets present
    miss = [s for s in EXPECTED_MAIN if s not in sheets]
    if miss: issues.append(f"MISSING main sheets: {miss}")
    miss_d = [s for s in EXPECTED_DECOMP if s not in sheets]
    if miss_d: issues.append(f"MISSING decomp sheets: {miss_d}")

    # empty / all-null sheets (stop_loss legitimately may be empty)
    for s in sheets:
        df = sdf(wb, s)
    ...
    # portfolio_history: drawdown + cum_pnl
    ...
    # leverage 0: liability/interest ~0 ; asset == equity
    ...
    # sector_weights row-sum + cadence
    ...
    # stock_decomp reconcile (weights)
    decomp_max = 0.0
    P = sdf(wb, "sector_weights"); De = sdf(wb, "sector_weights_stock_decomp")
    if P is not None and De is not None:
        Pn = _num(P.set_index(P.columns[0])); Dn = _num(De.set_index(De.columns[0]))
        g = defaultdict(list)
        for c in Dn.columns:
            if "/" in str(c):
                g[str(c).split("/")[0]].append(c)
        for sub, cs in g.items():
            if sub in Pn.columns:
                com = Dn.index.intersection(Pn.index)
                if len(com):
                    e = (Dn.loc[com, cs].sum(axis=1) - Pn.loc[com, sub]).abs().max()
                    if e == e:
                        decomp_max = max(decomp_max, e)
    if decomp_max > TOL_DECOMP:
        issues.append(f"stock_decomp weight reconcile err={decomp_max:.5f}")
...
def main():
    ap = argparse.ArgumentParser(description="Deep per-sheet verification of AEUS backtest Excel")
    ap.add_argument("date", nargs="?", default=None, help="YYYYMMDD (default: today)")
    ap.add_argument("--dir", default=DEFAULT_DIR, help="historical_runs/electric_utilities_strategy dir")
    args = ap.parse_args()
    D = args.date or pd.Timestamp.today().strftime("%Y%m%d")

    out = [f"AEUS Excel deep verification — date={D} dir={args.dir}"]
    total_bad = 0; total_files = 0
    for pat, lab in [
        (f"aeus_portfolio_*_v1_IS_batch_{D}_*.xlsx",        "V1 IS batch"),
        (f"aeus_portfolio_*_v2_IS_batch_{D}_*.xlsx",        "V2 IS batch"),
        (f"aeus_portfolio_*_v1_IS-OOS_tearsheet_{D}_*.xlsx","V1 IS-OOS"),
        (f"aeus_portfolio_*_v2_IS-OOS_tearsheet_{D}_*.xlsx","V2 IS-OOS"),
    ]:

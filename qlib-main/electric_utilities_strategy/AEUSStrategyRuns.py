"""
AEUSStrategyRuns — AEUS named parameter sets
============================================
Purpose-built parameter library for the AI Infra & Electric Utilities Strategy.
(Replaces the inherited AEUS sets, which referenced config paths — e.g.
``signals.value.*`` — that do not exist in AEUS.)
...
``AEUSBatchRun._group_of`` can bucket it.
...
# ---------------------------------------------------------------------------
# Apply / access helpers (same semantics as AEUS)
# ---------------------------------------------------------------------------
...
def get_param_set(name: str) -> dict:
    if name not in PARAM_SETS:
        raise KeyError(f"Unknown AEUS param set: {name!r}. Available: {sorted(PARAM_SETS)}")
    return PARAM_SETS[name]


# ---------------------------------------------------------------------------
# Weight helper (4 AEUS factors must sum to 1.0)
# ---------------------------------------------------------------------------
...
PARAM_SETS: Dict[str, Dict[str, Any]] = {
    ...
    # ===== Group N — intra-subsector purity tilt (AEUS_PLAN §2.5) ========
    # κ=0(默认 config)= AISS 静态板块内配比,逐 bit 回归锚;walk-forward 用
    # 这三组裁决生产 κ,不拍脑袋。clip 固定 0.40(单票倾斜上限)。
    "purity_tilt_off":   {"signals.purity_tilt.kappa": 0.0},
    "purity_tilt_03":    {"signals.purity_tilt.kappa": 0.3},
    "purity_tilt_05":    {"signals.purity_tilt.kappa": 0.5},
}
...
_PARAM_SET_DESCRIPTIONS: Dict[str, str] = {
    ...
    "purity_tilt_off":   "N1 purity tilt κ=0 — static intra-subsector base_w (regression anchor)",
    "purity_tilt_03":    "N2 purity tilt κ=0.3 — graph-driven intra-subsector amplification",
    "purity_tilt_05":    "N3 purity tilt κ=0.5 — aggressive purity amplification",
}


def list_param_sets() -> None:
    """Print all AEUS param sets grouped by letter."""
    by_group: Dict[str, List[str]] = {}
    for name in PARAM_SETS:
        g = _PARAM_SET_DESCRIPTIONS.get(name, "?")[0].upper()
        by_group.setdefault(g, []).append(name)
    print(f"AEUS PARAM SETS ({len(PARAM_SETS)} total)")

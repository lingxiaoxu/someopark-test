#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════
# daily_backtest.sh — AEUS V1 + V2 backtest / selection suite
# ═══════════════════════════════════════════════════════════════════════════
# Mirrors the SSRS daily_backtest flow, purpose-built for AEUS.  Runs after the
...
# Run from repo root:  bash qlib-main/electric_utilities_strategy/daily_backtest.sh
# ═══════════════════════════════════════════════════════════════════════════
...
if [ "$FORCE_RERUN" -eq 0 ]; then
    _done_log=$(grep -l "AEUS DAILY BACKTEST COMPLETE" \
        "$LOG_DIR"/aeus_daily_backtest_$(date +%Y%m%d)_*.log 2>/dev/null | head -1)
    if [ -n "$_done_log" ]; then
        echo "[$(date +%H:%M:%S)] AEUS daily_backtest already completed today" \
...
_OUT="$REPO_ROOT/historical_runs/electric_utilities_strategy"; _PDF="$SCRIPT_DIR/report/output"
log "── Artifacts (this run, $(date +%Y%m%d)) ──"
log "  V1 batch IS Excel    : $(ls "$_OUT"/aeus_portfolio_*_v1_IS_batch_$(date +%Y%m%d)_*.xlsx 2>/dev/null | wc -l | tr -d ' ')"
log "  V1 IS-OOS Excel       : $(ls "$_OUT"/aeus_portfolio_*_v1_IS-OOS_tearsheet_$(date +%Y%m%d)_*.xlsx 2>/dev/null | wc -l | tr -d ' ')"
log "  V2 batch IS Excel    : $(ls "$_OUT"/aeus_portfolio_*_v2_IS_batch_$(date +%Y%m%d)_*.xlsx 2>/dev/null | wc -l | tr -d ' ')"
log "  V2 IS-OOS Excel       : $(ls "$_OUT"/aeus_portfolio_*_v2_IS-OOS_tearsheet_$(date +%Y%m%d)_*.xlsx 2>/dev/null | wc -l | tr -d ' ')"
log "  WF diagnostic Excel   : $(ls "$_OUT"/wf_diagnostic_aeus_*_$(date +%Y%m%d)_*.xlsx 2>/dev/null | wc -l | tr -d ' ')"

...
用法（在 someopark-test/ 目录下）：
    ...
    python qlib-main/electric_utilities_strategy/backtest/trade_audit.py [--output-dir DIR]

2026-07-21: 全体包进 main() + __main__ 守卫——此前模块顶层执行,import 即跑全量
审计并覆写 historical_runs/ 的 csv(import 副作用地雷)。新增 --output-dir
(默认仍是 historical_runs/electric_utilities_strategy/,无参调用行为不变)。
"""
...
def main(output_dir: str | None = None) -> None:
    ...
    from electric_utilities_strategy.backtest.engine    import AEUSBacktest  # noqa: F401
    ...
    from electric_utilities_strategy.data.loader import load_returns
    ...
    # ── 2. Save to CSV ────────────────────────────────────────────────────────
    _out_dir = output_dir or os.path.join(REPO_ROOT, "historical_runs", "electric_utilities_strategy")
...
if __name__ == "__main__":
    import argparse
    _ap = argparse.ArgumentParser(description="AEUS trade audit replay")
    _ap.add_argument("--output-dir", default=None,
                     help="CSV output dir (default: historical_runs/electric_utilities_strategy/)")

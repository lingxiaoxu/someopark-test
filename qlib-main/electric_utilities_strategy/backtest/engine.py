...
Backtest Engine
===============
Event-driven monthly backtest for the AEUS strategy.
...
Primary path uses qlib's full backtest infrastructure:
    ...
    - AEUSWeightStrategy (qlib WeightStrategyBase): signal→weight logic
...
class BacktestResult:
    ...
    def summary(self) -> str:
        ...
        lines = [
            "=" * 60,
            "AEUS (AI Infra & Electric Utilities) BACKTEST SUMMARY",
            "=" * 60,
...
class AEUSBacktest:
    ...
    Usage:
        bt = AEUSBacktest(config)
        result = bt.run()
        print(result.summary())
    """

    def __init__(self, config: dict, mlflow_experiment: str = "electric_utilities_strategy_backtest"):
        self.cfg = config
    ...
    def run(
        ...
        _csm = self.sig_cfg.get("cs_momentum", {})
        signal_kwargs = {
            "signal_version": self.sig_cfg.get("signal_version", "v1"),
            "cs_lookback": _csm.get("lookback_months", 12),
            "cs_skip": _csm.get("skip_months", 1),
            "cs_zscore_window": _csm.get("zscore_window", 36),
        ...
        # IMPORTANT: param-set weights take PRECEDENCE over V2 defaults.
        # V2 only overrides weights if the param set didn't touch them (i.e.
        # sig_weights still equals the V1 config defaults).
        if signal_kwargs.get("signal_version") == "v2":
            _v2 = self.sig_cfg.get("v2", {})
            _v2_csm = _v2.get("cs_momentum", {})
            signal_kwargs["cs_lookback"] = _v2_csm.get("lookback_months", signal_kwargs.get("cs_lookback", 12))
            signal_kwargs["cs_skip"] = _v2_csm.get("skip_months", signal_kwargs.get("cs_skip", 1))
            signal_kwargs["cs_zscore_window"] = _v2_csm.get("zscore_window", signal_kwargs.get("cs_zscore_window", 36))
            if _v2.get("weights"):
                # Only apply V2 default weights when the param set didn't change
                # the core 4 weights. NOTE: sig_weights and self.cfg.signals.weights
                # are the SAME dict (mutated by setdefault above), so we compare
                # core keys against V2's defaults to detect param-set overrides.
                _v2_w = dict(_v2["weights"])
                _core_keys = list(_v2_w.keys())
                _core_match = all(
                    abs(float(sig_weights.get(k, 0)) - float(_v2_w.get(k, 0))) < 1e-9
                    for k in _core_keys
                )
                if _core_match:
                    sig_weights = _v2_w  # no param-set override → use V2 defaults

        composite, regime_monthly, components = compute_composite_signals(
            etf_prices,
            macro,
            weights=sig_weights,
            regime_multipliers=_regime_cfg.get("regime_weights"),
            defensive_tickers=_regime_cfg.get("defensive_sectors"),
            defensive_bonus=_regime_cfg.get("defensive_bonus_risk_off", 0.40),
        ...
        # ---------------------------------------------------------------
        # 2b. §2.5 purity tilt — SECOND PASS for the PnL path (AEUS extension)
        # ---------------------------------------------------------------
        # Signals above are computed on BASE baskets (one-pass); when κ>0 the
        # accounting/return path switches to graph-tilted baskets so walk-forward
        # can actually judge κ (Group N).  κ=0 → skipped → byte-identical to the
        # inherited AISS behaviour (regression anchor).  Same formula as
        # stock_decompose.apply_purity_tilt via loader.build_subsector_prices.
        _pt_cfg = self.sig_cfg.get("purity_tilt", {}) or {}
        _kappa = float(_pt_cfg.get("kappa", 0.0) or 0.0)
        if _kappa > 0:
            try:
                from ..data.loader import load_subsector_prices as _lsp
                _g = components.get("supply_chain")
                _tilted = _lsp(config=self.cfg, graph_scores=_g, kappa=_kappa,
                               tilt_clip=float(_pt_cfg.get("clip", 0.40)),
                               include_benchmark=False)
                _cols = [c for c in etf_prices.columns if c in _tilted.columns]
                if _cols:
                    etf_prices = _tilted[_cols].reindex(etf_prices.index).ffill()
                    logger.info("Purity tilt PnL pass: κ=%.2f baskets rebuilt (%d subsectors)",
                                _kappa, len(_cols))
            except Exception as _pt_err:  # noqa: BLE001
                logger.warning("Purity tilt basket rebuild failed (%s) — base baskets kept",
                               _pt_err)
        ...
        # κ>0 时 etf_prices 已是倾斜篮子 → 直接对它取收益(κ=0 时二者逐 bit 相同)
        etf_daily_ret = load_returns(etf_prices)[[t for t in etf_tickers
    ...
    def _run_qlib(
        ...
        Full qlib infrastructure backtest:
            ...
            AEUSWeightStrategy + SectorSimulatorExecutor + backtest_loop
        """
        from .qlib_adapter import SectorETFExchange, SectorSimulatorExecutor
        from ..portfolio.strategy import AEUSWeightStrategy
    ...
    def _record_experiment(
        ...
        try:
            ...
            # AEUS keeps mlruns INSIDE the strategy dir (never the shared qlib-main/mlruns).
            mlruns_path = Path(__file__).parent.parent.resolve() / "mlruns"
            mlruns_path.mkdir(parents=True, exist_ok=True)
            uri = f"sqlite:///{mlruns_path}/mlflow.db"
            exp_manager = MLflowExpManager(uri=uri, default_exp_name="aeus_backtest")
            ...
            with recorder_client.start(experiment_name=self._mlflow_experiment):
                ...
                recorder_client.set_tags(
                    bt_start=bt_start,
                    bt_end=bt_end,
                    strategy="electric_utilities_strategy",
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run AEUS backtest")
    ...
    # active_cfg: used for AEUSBacktest (strategy keys — overridden by param set)
    ...
    if args.walk_forward:
        from electric_utilities_strategy.walk_forward import WalkForwardAnalyzer, run_dual_mode
        wf_kwargs = dict(
        ...
        engine = AEUSBacktest(active_cfg)

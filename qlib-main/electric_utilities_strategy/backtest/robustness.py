...
4. someopark correlation    — Correlation of AEUS returns with pairs PnL series
...
def load_someopark_pnl(
    ...
    This is used to compute correlation between AEUS and
...
def someopark_correlation_analysis(
    electric_utilities_strategy_returns: pd.Series,
    someopark_returns: Optional[pd.Series] = None,
    strategy_performance_path: Optional[str] = None,
) -> Dict:
    """
    Compute correlation between AEUS and someopark pairs trading.
    ...
    aligned = pd.concat([
        electric_utilities_strategy_returns.rename("electric_utilities_strategy"),
    ...
    full_corr = float(aligned.corr().loc["electric_utilities_strategy", "someopark"])

    # Rolling 63-day correlation
    rolling_corr = (
        aligned["electric_utilities_strategy"]
    ...
    w = 0.5  # Equal weight
    combined = w * aligned["electric_utilities_strategy"] + (1 - w) * aligned["someopark"]
    vol_combined = float(combined.std() * np.sqrt(252))
    vol_sr = float(aligned["electric_utilities_strategy"].std() * np.sqrt(252))
    ...
    sr_sharpe = sharpe_ratio(aligned["electric_utilities_strategy"])
    ...
    return {
        ...
        "electric_utilities_strategy_sharpe": round(sr_sharpe, 4),

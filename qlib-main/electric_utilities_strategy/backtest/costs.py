...
# AEUS trades single stocks (via subsector baskets) — no fund management fee.
# Name kept (ETF_ANNUAL_FEE_BPS) so downstream defaults stay wired; value = 0.
ETF_ANNUAL_FEE_BPS: float = 0.0

# Per-stock liquidity tiers (single source of truth = universe.STOCK_TIER).
try:
    from ..data.universe import STOCK_TIER as TICKER_TIER, subsector_cost_bps, SUBSECTORS
except Exception:  # pragma: no cover - allow standalone import
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
    from electric_utilities_strategy.data.universe import (  # type: ignore
        STOCK_TIER as TICKER_TIER, subsector_cost_bps, SUBSECTORS,
    )


def get_one_way_cost_bps(asset: str) -> float:
    """One-way transaction cost (bps) for a subsector basket OR a single stock.

    A subsector's cost is the 80/15/5-blended cost of its three constituents
    (computed in universe.subsector_cost_bps); a raw ticker uses its own tier.
    """
    if asset in SUBSECTORS:
        return subsector_cost_bps(asset)
    tier = TICKER_TIER.get(asset, 2)  # Default to tier 2 if unknown

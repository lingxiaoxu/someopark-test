"""
AEUSdailySignal.py — 板块轮动每日信号生成器
=======================================================
Production daily runner for the AEUS strategy.
...
用法：
  conda run -n qlib_run --no-capture-output \\
    python qlib-main/electric_utilities_strategy/AEUSdailySignal.py \\
    --capital 1000000 [--date YYYY-MM-DD] [--dry-run] [--force-rebalance]
      [--value-source proxy]

目录（相对于 someopark-test/）：
  qlib-main/electric_utilities_strategy/AEUSdailySignal.py  ← 本文件
  qlib-main/electric_utilities_strategy/inventory_aeus.json
  qlib-main/electric_utilities_strategy/trading_signals/              ← JSON + TXT 报告
  qlib-main/electric_utilities_strategy/inventory_history/            ← 历史快照
  qlib-main/electric_utilities_strategy/data/cache/                   ← ETF 价格缓存
...
sys.path.insert(0, str(_QLIB_DIR))      # electric_utilities_strategy.* imports
sys.path.insert(0, str(_PROJECT_DIR))   # MacroStateStore

# ── Sector-rotation module imports ───────────────────────────────────────────
from electric_utilities_strategy.data.loader import load_config, load_prices, load_stock_prices
from electric_utilities_strategy.data.universe import get_tickers, all_tickers
from electric_utilities_strategy.stock_decompose import (
    decompose_to_stocks,
    build_stock_trades,
...
log = logging.getLogger("AEUSdailySignal")

# ── Constants ─────────────────────────────────────────────────────────────────
INVENTORY_PATH       = _THIS_DIR / "inventory_aeus.json"
...
def _load_ledger_equity_curve(min_points: int = 2):
    ...
    try:
        rows = {}
        for p in sorted((_THIS_DIR / "account_history").glob("account_aeus_*.json")):
            try:
                ...
                continue
        cur = _THIS_DIR / "account_aeus.json"
        if cur.exists():
...
def save_inventory(inv: dict, dry_run: bool = False):
    ...
    bak = INVENTORY_HISTORY_DIR / f"inventory_aeus_{ts}.json"
...
def _sync_cost_basis_from_ledger(inv: dict, snap_path=None) -> int:
    ...
    """
    try:
        acct_path = Path(__file__).resolve().parent / "account_aeus.json"
...
def _load_macro_fallback(start: str, end: str) -> pd.DataFrame:
    ...
    from electric_utilities_strategy.data.loader import load_macro_data
...
def _write_report_json(report: dict, signal_date: date) -> Path:
    ...
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = SIGNALS_DIR / f"aeus_daily_report_{signal_date.strftime('%Y%m%d')}_{ts}.json"
...
def _market_liquidity_outlook_w2() -> Optional[dict]:
...
def _write_report_txt(report: dict, signal_date: date) -> Path:
    SIGNALS_DIR.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = SIGNALS_DIR / f"aeus_daily_report_{signal_date.strftime('%Y%m%d')}_{ts}.txt"

    lines = []
    sep = "=" * 64

    lines.append(sep)
    lines.append(f"  AEUS DAILY SIGNAL  —  {signal_date}")
    lines.append(sep)
...
def run_daily_signal(
    ...
    config_path   : Path to config.yaml (default: electric_utilities_strategy/config.yaml).
    ...
    if not dry_run:
        try:
            ...
            _ca = _ca_run_for('aeus')
            if _ca.get('applied'):
                log.warning(f"[CA][aeus] {len(_ca['applied'])} corporate action(s) "
                            f"applied to inventory — see corporate_actions.log")
        except Exception as _ca_e:
            log.warning(f"[CA][aeus] check failed (non-fatal): {_ca_e}")
    ...
    try:
        ...
        try:
            from electric_utilities_strategy.smart_select import smart_param_select, save_state
    ...
    # (data/aeus_fetch_prices.load_prices_wide), so marks are current without any
    ...
    # AEUS's tradeable "asset" is the subsector basket; the executable layer is
    ...
    from electric_utilities_strategy.signals.composite import DEFAULT_REGIME_WEIGHT_MULTIPLIERS
    ...
    if ev_cfg.get("enabled", False):
        try:
            ...
            from electric_utilities_strategy.portfolio.risk import estimate_sector_betas as _esb
            # AEUS portfolio beta vs SPY on the RAW (pre-de-risk) target weights (§3)
            _aeus_beta = float("nan")
            ...
            if _bench_ret is not None and not daily_returns.empty:
                ...
                _aeus_beta = float((target_weights_raw *
            ...
            _state, _ev = _erd.process(
                signal_date, _prev, aiss_beta=_aeus_beta,
            ...
            _erd.log_evaluation(_ev, _state, "AEUS",
                                str(SIGNALS_DIR / "event_risk_heartbeat.log"),
                                extra=f"aiss_beta={_aeus_beta:.2f}")
    ...
    if not will_rebalance:
        ...
        from electric_utilities_strategy.portfolio.rebalance import apply_zscore_threshold_filter
        ...
        from electric_utilities_strategy.portfolio.rebalance import cap_turnover
    ...
    # SSRS trades 11 ETFs directly; AEUS's subsector is a synthetic basket, so the
    ...
    from electric_utilities_strategy.data import universe as _UNIV
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="AEUS Daily Signal Generator",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Standard daily run (proxy P/E, fast)
  conda run -n qlib_run python electric_utilities_strategy/AEUSdailySignal.py --capital 1000000

  # With real constituent P/E (slow on first run, cached afterwards)
  conda run -n qlib_run python electric_utilities_strategy/AEUSdailySignal.py \\
    --capital 1000000 --value-source constituents

  # Dry run for today
  conda run -n qlib_run python electric_utilities_strategy/AEUSdailySignal.py --dry-run
...
""",
    ...
    parser.add_argument(
        ...
        help="Path to config.yaml (default: electric_utilities_strategy/config.yaml)",
    )
    parser.add_argument(
    ...
    if report:
        print()
        print("=" * 64)
        print(f"  AEUS  —  {sig_date}  (dry_run={args.dry_run})")
        print("=" * 64)

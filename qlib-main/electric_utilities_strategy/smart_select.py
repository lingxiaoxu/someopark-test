...
def macro_positioning(
    ...
    与 macro_clusters.py 重建工件共用**同一持久化 encoder**(同一 latent 基底,
    绝不每日重训);训练矩阵/今日向量取根 MacroStateStore 的 23 维(caller 的
    macro_df 只有 10 列,历史病灶,不再用于本层);编码走 latent_of 完整变换链
    (prepass/log/标准化 —— 旧代码直塞生向量进 _encode,latent 是尺度噪音)。
    任何前提缺失 → available=False + reason,大声不出数,绝不 0 填。
    ...
    try:
        ...
        from electric_utilities_strategy.macro_clusters import (AE_ARTIFACT,
        ...
        if list(hist.columns) != list(feats):
            # encoder 与 store 特征集漂移(如 store 加列)→ 需重建,不硬编
...
def mcps_realtime_scores(
    ...
    try:
        ...
        from electric_utilities_strategy.macro_clusters import load_full_macro_history
...
def macro_weight_tilt(
    ...
    try:
        ...
        from electric_utilities_strategy.data.loader import load_prices

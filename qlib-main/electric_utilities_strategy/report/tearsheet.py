...
def _add_title_page(pdf: PdfPages, metrics: dict, config: dict,
    ...
    ax.text(0.040, 0.938, "AI Infra & Electric Utilities Strategy  —  Performance Tearsheet",
            transform=ax.transAxes, ha="left", va="center",
    ...
    # ── FOOTER ────────────────────────────────────────────────────────────────
    ax.text(0.5, 0.005,
            "Someo Park  ·  AI Infra & Electric Utilities Strategy v1.0  ·  "
...
def _add_benchmark_comparison_page(pdf, result, prices) -> None:
    """AEUS vs XLU / GRID / SPY — normalized equity overlay + win-criterion table.

    XLU & GRID are the strategy's mandate hurdle; this page makes the
    ...
    curves = {"AEUS": eq / eq.iloc[0]}
    rets = {"AEUS": eq.pct_change()}
    colors = {"AEUS": PALETTE["strategy"], "XLU": "#d62728", "GRID": "#ff7f0e", "SPY": PALETTE.get("benchmark", "#888")}
    ...
    fig = plt.figure(figsize=(14, 9))
    fig.patch.set_facecolor(PALETTE["bg"])
    fig.suptitle("AEUS vs Electric Utilities Benchmarks (XLU / GRID)", fontsize=13,
                 fontweight="bold", color=PALETTE["header_mid"], y=0.97)
    ax = fig.add_axes([0.07, 0.34, 0.88, 0.55])
    ax.set_facecolor(PALETTE["surface"])
    for name, c in curves.items():
        ax.plot(c.index, c.values, label=name, linewidth=2.0 if name == "AEUS" else 1.2,
                color=colors.get(name), alpha=1.0 if name == "AEUS" else 0.8)
    ax.set_yscale("log")
    ...
    # metrics table
    rows, cell = [], []
    a_cagr, a_sh, a_dd = _met(rets["AEUS"])
    ...
    beats_soxx = ("XLU" in rets) and (a_sh > _met(rets["XLU"])[1]) and (a_cagr > _met(rets["XLU"])[0])
    beats_smh = ("GRID" in rets) and (a_sh > _met(rets["GRID"])[1]) and (a_cagr > _met(rets["GRID"])[0])
    verdict = "PASS — AEUS beats XLU & GRID on Sharpe AND CAGR" if (beats_soxx and beats_smh) \
...
def generate_tearsheet(
    ...
    Parameters
    ----------
    result            : BacktestResult from AEUSBacktest.run()
    output_path       : Full PDF path (auto-generated if None)
    prices            : Raw prices DataFrame (for correlation page)
    daily_report_dir  : Directory containing aeus_daily_report_*.json files
                        (defaults to <repo>/trading_signals/)
    batch_df          : DataFrame from AEUSBatchRun (all 59 param sets)
    ...
    for d in search_dirs:
        if d.exists():
            candidates = sorted(d.glob("aeus_daily_report_*.json"), key=os.path.getmtime)
            if candidates:
    ...
    with PdfPages(str(output_path)) as pdf:
        ...
        # ── P2b: AEUS vs XLU / GRID / SPY (win-criterion overlay) ────────────
        ...
        # PDF metadata
        d = pdf.infodict()
        d["Title"]        = "AI Infra & Electric Utilities Strategy — Performance Tearsheet"
        d["Author"]       = "Someo Park"
        d["Subject"]      = "Institutional Sector ETF Rotation Backtest"
        d["Keywords"]     = "AEUS, ETF, risk parity, momentum, backtest"
...
def compute_mcps_rolling_equity(
    ...
    for name in set_names:
        try:
            ...
            res = AEUSBacktest(cfg).run(prices=prices, macro=macro)
...
def main() -> None:
    """CLI: run an AEUS backtest and emit a PDF tearsheet (vs XLU/GRID/SPY)."""
    import argparse
    import sys
    ...
    ap = argparse.ArgumentParser(description="AEUS PDF tearsheet")

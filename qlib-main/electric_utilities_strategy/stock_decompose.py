...
stock_decompose.py
==================
Decompose AEUS **subsector** target weights into the underlying **individual-stock**
holdings + trades — the executable layer one level below the subsector "asset".

Why this exists (the AEUS vs SSRS difference)
---------------------------------------------
SSRS trades 11 GICS **ETFs** directly, so its daily signal stops at the ETF level
— that *is* the tradeable instrument.  AEUS's tradeable "asset" is a *synthetic*
80/15/5 subsector basket, which is not directly executable.  So AEUS needs **one
...
def apply_purity_tilt(eff_w: Dict[str, float], subsector: str,
                      g_score: float, kappa: float,
                      clip: float = 0.40) -> Dict[str, float]:
    """AEUS_PLAN §2.5 第 3 层:板块内纯度倾斜(双层共用的唯一实现)。

        w_i ∝ eff_w_i × (1 + clamp(κ · purity_i · g_s, ±clip)),再归一。

    κ=0 时**逐 bit 返回原 dict**(回归锚:AISS 静态行为);g_score 用调用方给的
    上月末图谱分(PIT 与调仓同源)。clip 防单票被极端图谱分放大过界。
    loader.build_subsector_prices(回测篮子)与本模块 decompose_to_stocks
    (执行层)都调这一个函数 —— 两层逐 bit 一致,WF 才能裁决 κ(§2.5 设计洞修复)。
    """
    if not kappa or kappa == 0.0 or not eff_w:
        return eff_w
    g = float(g_score or 0.0)
    tilted = {}
    for tk, w in eff_w.items():
        adj = kappa * U.get_purity(tk, subsector) * g
        adj = max(-clip, min(clip, adj))
        tilted[tk] = w * (1.0 + adj)
    total = sum(tilted.values())
    if total <= 0:
        return eff_w
    return {tk: w / total for tk, w in tilted.items()}
...
def decompose_to_stocks(
    ...
    graph_scores : dict, optional
        {subsector: 上月末 supply_chain 图谱 z 分}(§2.5 purity tilt 的 g_s)。
    kappa : float
        纯度倾斜强度(config signals.purity_tilt.kappa;0 = AISS 静态行为)。
    ...
    for sub, sub_w in subsector_weights.items():
        ...
        # §2.5 第 3 层:图谱驱动纯度倾斜(κ=0 → eff 原样,AISS 行为)
        if graph_scores is not None and kappa:
            eff = apply_purity_tilt(eff, sub, graph_scores.get(sub, 0.0),
                                    kappa, clip=tilt_clip)

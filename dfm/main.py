...
def main():
    ...
    parser.add_argument('--force_mlp', action='store_true',
                        help='use the MLP FactorScoreNetwork even when a 2D UNet reshape exists')
    ...
    if args.force_mlp:
        ...
        logger.info(f"--force_mlp: using MLP FactorScoreNetwork at d={d}")
    rd = synthetic_config.get('reshape_dims')
    if not args.force_mlp and (rd is None or rd[0] * rd[1] != d):
        h = 1
        while (h * 2) * (h * 2) <= d:
            h *= 2
        while h > 1 and d % h != 0:

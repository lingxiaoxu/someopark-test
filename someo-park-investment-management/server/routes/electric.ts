/**
 * AI Electric Utilities Strategy (AEUS) API routes — /api/aeus/*
 *
 * Twin of the SSRS (sector rotation) routes; reads from
 * qlib-main/electric_utilities_strategy/ and historical_runs/electric_utilities_strategy/.
 *
 * IMPORTANT: AEUS trades individual STOCKS grouped into subsectors. The
 * subsector layer is NOT tradable, so holdings/universe endpoints always
 * expose stock-level positions (from inventory_aeus.json -> stock_holdings),
...
// ── Path helpers ────────────────────────────────────────────────────
const AEUS_DIR = () => getBackendPath('qlib-main/electric_utilities_strategy');
const AEUS_SIGNALS = () => path.join(AEUS_DIR(), 'trading_signals');
const AEUS_BACKTEST = () => path.join(AEUS_DIR(), 'backtest_results');
const AEUS_HISTORY = () => getBackendPath('historical_runs/electric_utilities_strategy');
const AEUS_REPORT = () => path.join(AEUS_DIR(), 'report/output');
const AEUS_INV_HISTORY = () => path.join(AEUS_DIR(), 'inventory_history');
...
// GET /api/aeus/inventory — full inventory + derived stock-level view
router.get('/inventory', async (_req, res) => {
  try {
    const filePath = path.join(AEUS_DIR(), 'inventory_aeus.json');
    const data = await readJsonFile(filePath);
    const view = buildStockView(data);
    res.json({ ...data, stock_view: view });
  } catch (err: any) {
    // Pre-go-live (before 2026-09-01 首仓): inventory_aeus.json does not exist
    // yet — return a valid empty inventory instead of a 500 so the viewer shows
    // its "暂无持仓" empty state (mirrors the inventory-history UX).
    res.json({
      available: false, empty: true, as_of: null, last_updated: null,
      param_set: '', signal_version: 'v1', regime: null,
      holdings: {}, stock_holdings: {}, cash_weight: 1.0,
      stock_view: { subsectors: [], stocks: [] },
      note: 'go-live 前(未建仓)— inventory_aeus.json 尚不存在',
    });
  }
});

// GET /api/aeus/inventory/history
router.get('/inventory/history', async (_req, res) => {
  try {
    const files = await listFiles(AEUS_INV_HISTORY(), 'inventory_aeus_*.json');
...
// GET /api/aeus/inventory/history/:filename
router.get('/inventory/history/:filename', async (req, res) => {
  try {
    const filename = path.basename(req.params.filename);
    const filePath = path.join(AEUS_INV_HISTORY(), filename);
    const data = await readJsonFile(filePath);
...
// GET /api/aeus/signals/latest
router.get('/signals/latest', async (_req, res) => {
  try {
    const latest = await getLatestFile(AEUS_SIGNALS(), 'aeus_daily_report_*.json');
    ...
    res.json({
      ...
      stock_universe: data.stock_universe || [],    // ALL 10 subsectors × member stocks (incl 0%/reserve)
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
// GET /api/aeus/daily-report/latest
...
// GET /api/aeus/daily-report/latest/txt
router.get('/daily-report/latest/txt', async (_req, res) => {
  try {
    const latest = await getLatestFile(AEUS_SIGNALS(), 'aeus_daily_report_*.txt');
    if (!latest) return res.type('text').send('No AEUS daily report available');
...
// GET /api/aeus/stock-universe — FULL tradable universe (all 10 subsectors ×
...
router.get('/stock-universe', async (_req, res) => {
  try {
    ...
    const inv = await readJsonSafe(path.join(AEUS_DIR(), 'inventory_aeus.json')) || {};
    ...
    if (report?.stock_universe?.length) {
      ...
      const subsectors = report.stock_universe.map((g: any) => ({
        ...
        stocks: (g.stocks || []).map((s: any) => {
          // A stock can be a MEMBER of multiple subsectors, but its shares
          // are held via ONE subsector only (inventory `subsectors` field).
          // Attribute the shares/market-value to that owning subsector so a
          // 0%-weight subsector shows "—" instead of duplicating the owning
          // subsector's holding. Fallback to per-context weight for older
          // data without a `subsectors` field.
...
// GET /api/aeus/wf/summary
router.get('/wf/summary', async (_req, res) => {
  try {
    const filePath = path.join(AEUS_BACKTEST(), 'wf_fold_detail.json');
    const data = await readJsonFile(filePath);
    // universe counts (subsectors + distinct stocks) from the latest daily report
    let n_subsectors = 10, n_stocks = 0;
    try {
      const latest = await getLatestFile(AEUS_SIGNALS(), 'aeus_daily_report_*.json');
...
// GET /api/aeus/wf/param-oos
router.get('/wf/param-oos', async (_req, res) => {
  try {
    const batchFile = await findLatestFile(AEUS_BACKTEST(), 'aeus_batch_summary_*.csv');
    const rows = await parseCsvFile(batchFile);
    let regimeData: Record<string, any> = {};
    try {
      const regimePath = path.join(AEUS_BACKTEST(), 'param_oos_by_regime.json');
...
// GET /api/aeus/equity-curve?param=X&version=v1
router.get('/equity-curve', async (req, res) => {
  try {
    const selPath = path.join(AEUS_DIR(), 'selected_param_set.json');
    ...
    let files = await listFiles(AEUS_HISTORY(), `aeus_portfolio_${param}_${ver}_IS-OOS_*.xlsx`);
    if (files.length === 0) {
      files = await listFiles(AEUS_HISTORY(), `aeus_portfolio_${param}_*.xlsx`);
    }
    if (files.length === 0) {
    ...
    try {
      const wfPath = path.join(AEUS_BACKTEST(), `wf_fold_detail_${ver}.json`);
      const wfData = await readJsonSafe(wfPath) || await readJsonSafe(path.join(AEUS_BACKTEST(), 'wf_fold_detail.json'));
...
const PORTFOLIO_RE = /aeus_portfolio_(.+)_(v[12])_(IS(?:-OOS)?)_(batch|select|tearsheet|backtest)_(\d{8}_\d{6})\.xlsx/;

// GET /api/aeus/portfolio-history/list
router.get('/portfolio-history/list', async (_req, res) => {
  try {
    const files = await listFiles(AEUS_HISTORY(), 'aeus_portfolio_*.xlsx');
...
// GET /api/aeus/portfolio-history/:filename/sheets
router.get('/portfolio-history/:filename/sheets', async (req, res) => {
  try {
    const filename = path.basename(req.params.filename);
    const filePath = path.join(AEUS_HISTORY(), filename);
...
// GET /api/aeus/portfolio-history/:filename/:sheet
...
// GET /api/aeus/diagnostic/latest
router.get('/diagnostic/latest', async (_req, res) => {
  try {
    const files = await listFiles(AEUS_HISTORY(), 'wf_diagnostic_aeus_*.xlsx');
...
// GET /api/aeus/diagnostic/latest/:sheet
...
// GET /api/aeus/tearsheet/list
router.get('/tearsheet/list', async (_req, res) => {
  try {
    const files = await listFiles(AEUS_REPORT(), '*.pdf');
    const result = files.map(f => ({
      filename: path.basename(f),
...
// GET /api/aeus/tearsheet/:filename
router.get('/tearsheet/:filename', async (req, res) => {
  try {
    const filename = path.basename(req.params.filename);
    const filePath = path.join(AEUS_REPORT(), filename);
...
// GET /api/aeus/smart-select
router.get('/smart-select', async (_req, res) => {
  try {
    const latest = await getLatestFile(AEUS_SIGNALS(), 'aeus_daily_report_*.json');
    const sel = await readJsonSafe(path.join(AEUS_DIR(), 'selected_param_set.json'));
...
// GET /api/aeus/files/list
router.get('/files/list', async (_req, res) => {
  try {
    const files = await listFiles(AEUS_HISTORY(), '*.xlsx');
...
// GET /api/aeus/strategy-performance
router.get('/strategy-performance', async (_req, res) => {
  try {
    const sel = await readJsonSafe(path.join(AEUS_DIR(), 'selected_param_set.json'));
    const param = (sel?.param_set) || 'balanced_four';

    const v1Files = await listFiles(AEUS_HISTORY(), `aeus_portfolio_${param}_v1_*.xlsx`);
    const v2Files = await listFiles(AEUS_HISTORY(), `aeus_portfolio_${param}_v2_*.xlsx`);
...
// GET /api/aeus/params/list — list available param sets
router.get('/params/list', async (_req, res) => {
  try {
    const topPath = path.join(AEUS_BACKTEST(), 'top_candidates.json');

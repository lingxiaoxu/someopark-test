...
const VIEWS: Record<string, { file: string; about: string }> = {
  ...
  predictions: { file: 'upcoming.json',            about: 'today/upcoming matches: model 3-way + O2.5/BTTS, de-vig book, real Kalshi/Poly US asks, edges, AND the decision (value pick + stake + the dual-口径 plan: entry now + planned smart-exit cash-out). KNOCKOUT matches ALSO carry an `advance` block = the 2-way "who advances" market (model home/away incl. extra time + penalties, Kalshi/Poly US advance asks, edge, full decision); the dashboard 常规时间/晋级 (Regulation Time / Advances) toggle swaps the card to it. From the knockout stage a prediction is judged right/wrong on who ACTUALLY ADVANCED (2-way); group stage on the 90\' result' },
  schedule:    { file: 'upcoming.json',            about: 'fixture schedule with kickoff times (ET/PT) + recently finished matches (FT + score)' },
  inplay:      { file: 'inplay_live.json',         about: 'LIVE matches now: live 3-way, xG, remaining goals, + 15 in-play tactics (8 data-mined incl. the overshoot/smart-exit take-profit) + the OVER/UNDER totals market. Each opportunity carries an `intent` ("manage" = sell/lock a HELD position, "entry" = new value entry, "event" = red card / goal over-reaction), a CONFIDENCE tier (`confidence` high/med/low, from the validated effectiveness study) and a STAKING gate (`actionable` + `stake_usd`, else advisory — keeps every signal but only stakes the ones that clear the gate). Each live match also carries a `hedge` object — when we hold our pre-match DIRECTIONAL value bet, it gives the DRAW-protection: how many draw contracts to buy (break-even `b` = cost/(100−draw¢), and full hedge), a 3-state payoff matrix (home/draw/away win), `lead_state` (leading/level/behind), and our `entry_c`. The smart-money cross-validation only flags an entry when the book AND the model both beat the tradable venue price (venue lagging), agreeing with the model rather than chasing the book. This view is the 90\' 3-way; KNOCKOUT ties also run a PARALLEL 2-way "who advances" in-play stack — use view="inplay_advance" for it.' },
  inplay_advance: { file: 'inplay_live_advance.json', about: 'LIVE KNOCKOUT matches, 2-way "WHO ADVANCES" (home/away, no draw, incl. extra time + penalties): live advance probability recomputed each minute through regulation→ET→penalties (with reg/et/pens split + extra-time fatigue + sequential best-of-5/sudden-death shootout DP), advance-market venue ¢, advance-version tactics, and a 2-state `hedge_advance` (buy the OPPONENT\'s advance leg to protect a held side). Parallel to the 3-way inplay view; empty when no knockout match is live.' },
  ...
  risk:        { file: 'risk_report.json',         about: 'pre-trade gates, venue balances (Kalshi/Poly), $1 order cap, API budget/health, calibration gate, blocked summary. NOTE: this risk content is now surfaced INSIDE the merged "Venues & API" view (venue balances + trading gates + risk controls + API budget, categorized in one artifact) — there is no longer a standalone Risk Report card' },
  ...
  overview:    { file: 'frontend_overview.json',   about: 'system overview: interfaces, modes, schedule, inputs/outputs, value, state-aware headline. The System Overview and Model Notes (model methodology) are now ONE merged "System & Model Notes" view (overview + collapsible methodology sections)' },
...
export const predictionMarketTool: AgentTool = {
  definition: {
    ...
    description:
      ...
      'block in view="predictions" and view="inplay_advance"; from the knockout round win/loss is judged on who ' +
      'actually advanced.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  async execute({ view, top, team }) {
    ...
    if (view === 'inplay_advance') {
      if (!data || !data.n_live || !(data.matches || []).length) {
        return { n_live: 0, matches: [],
          message: 'No knockout match is live right now, so there is no 2-way "who advances" in-play view — ' +
            'the advance stack only runs during a live knockout tie. Use view="predictions" (advance block) ' +
            'for upcoming knockout matches.' }
      }
      return data
    }

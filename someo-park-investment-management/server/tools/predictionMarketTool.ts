...
const VIEWS: Record<string, { file: string; about: string }> = {
  ...
  reach_round: { file: 'reach_round.json',         about: 'reach-round (晋级盘): each of the 48 teams’ probability to reach each round (group-advance(R32) / R16 / QF / SF / final), model % vs Kalshi¢ (KXWCROUND) vs Poly¢ + edge; refreshed on every settle. Each team row ALSO carries group-stage form: group_gd (net goal difference), group_played (group matches played, of 3), group_rank (current in-group rank 1–4) — the UI shows them as "+1 (2=1) (#3)" = GD +1, (played=remaining where remaining=3−played), (#rank). Computed live from results, reconciled to the official group table. A team drawn into a published knockout fixture is pinned to 100% for that round even pre-kickoff, so model % can be 100% with a negative GD' },

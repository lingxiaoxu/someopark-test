// server/utils/controllerNavData.ts — 看板 HTTP 与聊天工具共用的只读 NAV 数据
...
/** 原 HTTP 路由明确返回的 JSON 错误;其余读盘错误保留原异常路径。 */
export class NavDataError extends Error {
  constructor(public readonly status: number, message: string) {
    super(message);
    this.name = 'NavDataError';
  }
}

// "今天"必须按 ET 算(2026-08-14 bug):toISOString 是 UTC,20:00 ET 后 UTC 已翻日,
// /stream 默认日指向不存在的明天文件、/prev-close 把**今天**的收盘当"昨收"返回。
export const etToday = (now: Date = new Date()) => new Intl.DateTimeFormat('en-CA', { timeZone: 'America/New_York' })
  .format(now).replace(/-/g, '');

// 最新全层级值(前端 45s 轮询)
export function readNavLatest() {
  const p = path.join(OUT, 'nav_latest.json');
  if (!fs.existsSync(p)) throw new NavDataError(404, 'controller not running yet');
  return JSON.parse(fs.readFileSync(p, 'utf-8'));
}

// 当日分钟流(频率子采样在前端做;date=YYYYMMDD 默认今天;
// date=latest → 最近一个有数据(≥2 行)的交易时段,闭市回看用)
export function readNavStream(requestedDate?: string) {
  let date = requestedDate === undefined ? etToday() : requestedDate;
  if (date === 'latest') {
  ...
  return { date, rows: readStreamMerged(date) };
}
...
export function readNavPrevClose(now: Date = new Date()) {
  if (!fs.existsSync(OUT)) throw new NavDataError(404, 'no output dir');
  const today = etToday(now);
  const days = fs.readdirSync(OUT)
    .map(f => /^nav_stream_(\d{8})\.csv$/.exec(f)?.[1])
    .filter((d): d is string => !!d && d < today)
    .sort();
  if (!days.length) return { date: null, values: {} };
  ...
  try {
    ...
    if (stDay === today && bv && Object.keys(bv).length) {
      ...
      if (Object.keys(vals).length) return { date, values: vals, src: 'day_state' };
    }
  ...
  return { date, values, src: 'stream_last' };
}
...
export function readNavMirror() {
  const p = path.join(REPO, 'trading_quantconnect', 'state', 'exporter_state.json');
  if (!fs.existsSync(p)) throw new NavDataError(404, 'no exporter state');
  try {
    const s = JSON.parse(fs.readFileSync(p, 'utf-8'));
    if (!s.scalars) throw new NavDataError(500, 'exporter_state has no scalars');
    ...
    if (e instanceof NavDataError) throw e;
    throw new NavDataError(500, String(e));
  }
}
...
function businessDaysSince(dateIso: string, now: Date): number | null {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateIso)) return null;
  const today = etToday(now);                               // YYYYMMDD(ET)
...
export function readNavReconcile(now: Date = new Date()) {
  if (!fs.existsSync(OUT)) throw new NavDataError(404, 'no output dir');
  const files = fs.readdirSync(OUT).filter(f => f.startsWith('reconcile_')).sort();
  if (!files.length) return { verdict: 'none', age_bdays: null, stale: true };
  const rep = JSON.parse(fs.readFileSync(path.join(OUT, files[files.length - 1]), 'utf-8'));
  const age = businessDaysSince(String(rep.date ?? ''), now);
  return { ...rep, age_bdays: age, stale: age === null || age > 1 };
}

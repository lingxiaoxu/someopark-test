"""
MTFSWalkForward.py — Walk-forward optimization for MTFS (Momentum Trend Following) strategy.

Methodology:
  - Expanding training window (anchored at auto-computed train_start)
  - Equal trading-day OOS windows (NYSE calendar aware)
  - Each training window runs the 29/30-param grid search (Step 1)
  - Best param_set per pair selected using Deflated Sharpe Ratio (DSR) to
    correct for multiple-comparison bias (N_TRIALS trials per pair)
...
Window design (default: 18mo train, 6 windows × 27 NYSE trading days):
  OOS period  = last 162 NYSE trading days ending at most-recent available date
  ...
  Example expanding (run on 2026-03-07, last data = 2026-03-06):
    train_start = 2024-06-XX  (18mo before first OOS day, fixed for all windows)
    Window 1: train 2024-06-XX -> 2025-08-XX  |  OOS 2025-08-XX -> 2025-09-XX
    Window 2: train 2024-06-XX -> 2025-09-XX  |  OOS 2025-09-XX -> 2025-10-XX
    ...
    Window 6: train 2024-06-XX -> 2026-02-XX  |  OOS 2026-02-XX -> 2026-03-06

  Example rolling (same dates):
    Window 1: train 2024-06-XX -> 2025-08-XX  |  OOS 2025-08-XX -> 2025-09-XX
    Window 2: train 2024-07-XX -> 2025-09-XX  |  OOS 2025-09-XX -> 2025-10-XX
...
MTFS-specific notes:
  ...
  - Pairs same 15 as MRPT universe

Usage:
  export $(cat .env | xargs) && conda run -n someopark_run python MTFSWalkForward.py [options]
...
Options:
  ...
  --oos-days N        Total OOS trading days across all windows (default: 162 = 6×27)
  ...
  --output-dir DIR    Where to write results (default: historical_runs/walk_forward_mtfs/)
...
def build_windows(last_date=None, n_windows=6, oos_days=162, train_months=18,
                  mode='expanding'):
    """
    ...
    oos_days : int
        Total OOS trading days across all windows (default 162 = 6×27).
    train_months : int
...
def run_grid_for_window(window, output_dir, skip_if_exists=False):
    """
    Run N-param grid search over training window.
    Returns path to the mtfs_strategy_summary CSV for this window.
    """
    ...
    summary_pattern = os.path.join(window_dir, 'mtfs_strategy_summary_*.csv')
    ...
    log.info(f'  [Window {window["window_idx"]}] Running MTFS grid search '
             f'{window["train_start"]} → {window["train_end"]} ...')

    _run_grid_direct(train_cfg, window, window_dir)

    # Find summary CSV written by this run (latest in window_dir)
    summaries = sorted(glob.glob(os.path.join(window_dir, 'mtfs_strategy_summary_*.csv')),
                       key=os.path.getmtime, reverse=True)
    if not summaries:
        raise RuntimeError(f'Grid search produced no MTFS summary CSV in {window_dir}')
    return summaries[0]
...
def _run_grid_direct(train_cfg, window, window_dir):
    ...
    Mirrors the core loop of run_from_config() in PortfolioMTFSStrategyRuns but
    redirects output to the walk-forward window directory.
    """
    ...
    if all_results:
        ...
        summary_path = os.path.join(window_dir, f'mtfs_strategy_summary_{ts}.csv')
        summary_df.to_csv(summary_path, index=False)
        log.info(f'    MTFS grid summary → {summary_path}')
    else:
        raise RuntimeError('No successful runs in MTFS grid search')


# ── Select best params per pair with DSR filter ───────────────────────────────

def select_pairs_with_dsr(summary_csv, window, n_trials=None):
    """
    ...
    log.info(f'\n  ── Pair selection (DSR filter, n_trials={n_trials}) ──')
    selected = []
    excluded = []
...
def run_test_window(window, selected_pairs, window_dir):
    """
    Run selected pairs with their best MTFS params on the OOS test window.
    Uses full training data as warmup (momentum indicators need history to converge),
    with actual trading starting only at test_start.
    ...
    # MTFS momentum indicators (SMA, VAMS, rolling correlations) need sufficient
    # history to converge — train data provides this warmup naturally.
    start_date       = window['train_start']
    ...
    log.info(f'  [Window {window["window_idx"]}] Running MTFS OOS test: {run_label}')
...
def build_oos_curve(window_results):
    ...
    Returns a DataFrame with columns: Date, OOS_Equity, OOS_DailyPnL, Window, n_pairs.
    """
    segments = []
    for winfo, result in window_results:
        ...
        eq_oos['Window'] = winfo['window_idx']
        eq_oos['n_pairs'] = len(winfo.get('selected_pairs', []))
        segments.append(eq_oos)
...
def _try_load_existing_oos(window, window_dir):
    ...
    pattern = os.path.join(window_dir, 'historical_runs',
                           f'portfolio_history_MTFS_wf_test_window{window["window_idx"]:02d}_*.xlsx')
...
def main():
    parser = argparse.ArgumentParser(description='Walk-forward optimization for MTFS momentum strategy')
    ...
    parser.add_argument('--oos-days',    type=int, default=162,
                        help='Total OOS NYSE trading days across all windows (default: 162 = 6×27)')
    ...
    parser.add_argument('--output-dir', default=None,
                        help='Output directory (default: historical_runs/walk_forward_mtfs/)')
    parser.add_argument('--skip-grid',  action='store_true',
                        help='Skip grid search if MTFS summary CSV already exists for a window')
    ...
    output_dir = args.output_dir or os.path.join(BASE_DIR, 'historical_runs', 'walk_forward_mtfs')
    ...
    log.info('=' * 65)
    log.info('MTFS Walk-Forward Optimization')
    log.info(f'  Mode:        {args.mode}')
    log.info(f'  Param sets:  {n_trials} (DSR n_trials)')
    ...
    for window in windows:
        ...
        selected_pairs, selection_df = select_pairs_with_dsr(
            summary_csv, window, n_trials=n_trials
        )

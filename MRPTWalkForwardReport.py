#!/usr/bin/env python3
"""
MRPTWalkForwardReport.py — Walk-forward OOS performance report generator.

Automatically finds the most recent walk-forward run in historical_runs/walk_forward/,
reads all OOS test Excel files, and produces:
  ...
  4. Full report saved to historical_runs/walk_forward/oos_report_<ts>.txt

Usage:
    conda run -n someopark_run python MRPTWalkForwardReport.py
    conda run -n someopark_run python MRPTWalkForwardReport.py --wf-dir historical_runs/walk_forward
...
def _find_oos_windows(wf_dir, run_prefix=None):
    ...
    for wdir in sorted(all_dirs):
        ...
        # Find OOS test xlsx (in wdir/historical_runs/)
...
def _load_window_data(w):
    ...
    interest = _read('interest_expense_history') if 'interest_expense_history' in \
        pd.ExcelFile(xlsx).sheet_names else pd.DataFrame()
...
def _pair_stats(pair, all_dod, all_trades, all_interest_total):
    ...
    dd   = cum - peak
    max_dd = float(dd.min())
    max_dd_pct = max_dd / 500_000  # relative to starting equity
...
def generate_report(wf_dir=None, run_prefix=None):
    ...
    print(f'Found {len(windows)} OOS windows in: {wf_dir}')
    ...
    lines = []
    lines.append('=' * 72)
    lines.append('WALK-FORWARD OOS REPORT')
    lines.append(f'Generated: {datetime.now().strftime("%Y-%m-%d %H:%M")}')
    lines.append(f'Windows:   {len(windows_data)}')
    lines.append('=' * 72)
    lines.append('')
    ...
    lines.append('-' * 72)
    lines.append(f"  Total OOS PnL:      ${cs['total_pnl']:>+12,.0f}  (equity-based, net of interest)")
    lines.append(f"  OOS Sharpe:         {cs['sharpe']:>8.3f}")
    lines.append(f"  OOS Max Drawdown:   ${cs['max_dd']:>12,.0f}  ({cs['max_dd_pct']:.2%})")
    lines.append(f"  OOS Trading Days:   {cs['n_days']:>8}")
    ...
    if total_interest != 0:
        ...
        lines.append(f"  Gross trading PnL:  ${cs['total_pnl'] + total_interest:>+12,.0f}  (before interest)")
        lines.append('')
    ...
    for pair in ALL_PAIRS:
        ps = _pair_stats(pair, all_dod, all_trades, total_interest)
        if ps:
            pair_rows.append(ps)
        else:
    ...
    lines.append('PER-WINDOW PAIR SELECTION  (DSR-filtered)')
    lines.append('-' * 72)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Walk-forward OOS report generator')
    parser.add_argument('--wf-dir', default=None,
                        help='Path to walk_forward output dir (default: historical_runs/walk_forward/)')
    parser.add_argument('--run-prefix', default=None,
                        help='Filter windows by train_start prefix e.g. "2024-02-01"')

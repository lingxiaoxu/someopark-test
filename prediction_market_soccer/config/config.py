"""Central, isolated configuration for the prediction_market_soccer project.

Everything this project needs (paths, model parameters, venue settings) is
resolved RELATIVE to ``prediction_market_soccer/`` so the project never reaches into
the surrounding someopark-test repo. All output is written under
``prediction_market_soccer/data/`` and (optionally) mirrored to the frontend.
...
# Project root = prediction_market_soccer/  (this file is prediction_market_soccer/config/config.py)
...
class Paths:
    """All filesystem locations, anchored under prediction_market_soccer/."""
    ...
    # Frontend hand-off location — the soccer module writes ONLY into its own
    # public/data/soccer/ namespace so it can never collide with the World Cup
    # module's root-level JSON files (TRANSFORM_PLAN §3.7 / §4).
    frontend_data: Path = REPO_ROOT / "someo-park-investment-management" / "public" / "data" / "soccer"

    def ensure(self) -> None:
        """Create the writable data dirs if missing (idempotent).

        Also creates frontend_data (public/data/soccer/): the WC version relied on
        the dir pre-existing; the soccer namespace subdir is ours to create
        (TRANSFORM_PLAN §3.7 — one-line root fix for _write_both/run_model).
        """
...
class ModelConfig:
    ...
    # ── Match-motivation λ tilt (club season-incentive psychology) ──────────────
    # OFF, and off twice over. LIVE betting-only when on (never in calibration / OOS / the
    # trade-grade gate). The five signals in model/motivation.py — relegation fight, title
    # run-in, dead rubber, pre-continental rotation, clinched cup rotation — are plausible
    # club analogues of the WC edition's group-progression psychology, but NOT ONE of them
    # has been validated on club data: the constants below are hand-set placeholders, not
    # fitted numbers, and the WC values they replaced were tuned on a 48-team tournament
    # with three group games. Enabling this needs a per-league PIT study over enough settled
    # run-in fixtures to show the tilt beats the flat model out of sample. Until then the
    # base model prices every club match clean.
    motiv_enabled: bool = False
    motiv_weight: float = 0.0          # scales EVERY tilt; 0 = exact no-op even if enabled
    motiv_late_frac: float = 0.75      # "run-in" = this fraction of a club's rounds played
    motiv_min_played: int = 6          # below this the PIT table is too noisy to read a zone
    motiv_zone_slack: int = 2          # places above the drop/title cut that still count as in it
    motiv_relegation_attack: float = 1.06  # relegation fight: attacking λ ×
    motiv_relegation_def: float = 1.04     # …and the λ it concedes (chasing leaves gaps behind)
    motiv_title_attack: float = 1.05       # title run-in: attacking λ ×
    motiv_dead_rubber_attack: float = 0.95  # nothing left to play for: attacking λ ×
    motiv_rotation_attack: float = 0.93     # rested XI (pre-continental, or cup already clinched)
    motiv_europe_window_days: int = 3       # a continental tie within N days ⇒ domestic rotation
    motiv_clamp: float = 0.15               # hard bound on the composite tilt per side
    ...
    # Season Monte-Carlo ONLY: per-path club rating uncertainty (rating units, held
    # for a whole simulated season, shrunk by √(6/(6+matches played)) as the table
    # answers the question). 0 = the old point-estimate sim, whose champion odds
    # compounded a fixed per-match edge over 34 rounds into false near-certainty
    # (Bayern 99.99% against an 87¢ market). At 0.9, a pre-season league lands next
    # to the market on all three checked boards (Bayern 84 vs 87¢, Arsenal 49 vs
    # 52¢, Barcelona 63 vs 59¢) without touching single-match pricing.
    season_rating_sigma: float = 0.90
    ...
    # Alt-data λ multipliers. These are now PER-COMPETITION (data/priors/
    # league_altdata.json, fitted by ops/fit_altdata_weights on each competition's
    # own 2025-26 history) — the values here are only the fallback for a
    # competition with no fitted entry yet. The WC pair (0.45/0.25) was hand-set on
    # 19 international matches and measurably hurt club prediction as a global
    # constant (test Brier 0.6355 vs 0.6278 with it off), which is why the weight
    # now belongs to the competition rather than to the model.
    oppadj_def_weight: float = 0.20  # a defensively-resilient team lowers its opponent's λ
    oppadj_off_weight: float = 0.11  # a team that scores vs strong defences raises its own λ
...
class SoccerConfig:
    ...
    # Competitions come from config.leagues.REGISTRY (12 launch comps, single
    # standard). league_id is retained ONLY as a legacy default for idle copied
    # code paths; every production path iterates the registry instead.
    league_id: int = 39         # legacy default (EPL); production uses leagues.active()
    season: int = 2026

    # Budget guard rails — API-Football bills PER DAY (UTC reset), so the daily cap is the
    # real guard. Soccer self-limits to 6,500 of the 7,500 Pro plan, leaving 1,000
    # headroom for the (idle) World Cup module sharing the same key (§4/§6.1).
    daily_budget: int = 6500            # self-limit (plan §6.1); Pro plan: 7,500/day
    monthly_budget: int = 200000        # loose backstop (~7500/day × 30); daily guard binds
    max_requests_per_run: int = 250     # hard stop per sync invocation (safety).
    # 60 was WC-sized (104 fixtures total); a club weekend finishes ~40 matches
    # × 3 detail calls and the daily 07:30 run is THE big ingest of the day.
    # 250 still sits far under the 6,500 daily self-limit.
...
class DecisionConfig:
    ...
    # Sanity ceiling on the claimed edge. A liquid 3-way that disagrees with us by
    # more than this is telling us our own number is wrong (thin-prior clubs in the
    # European qualifying rounds produced +0.25..+0.30 "edges" and lost every one).
    # 0 disables the guard.
    max_net_edge: float = 0.22
    ...
    # Motivation CONVICTION bet (decision behaviour): force a bet on a motivated top pick even
    # with no market value, overriding the usual edge gate. OFF for clubs, and the third
    # independent latch on the same feature. The WC thesis behind it — the market under-rates
    # an all-out wounded favourite in a group game — rested on tournament examples, not on
    # anything measured here; model/motivation.py accordingly never reports a conviction_side,
    # so the override cannot fire in this module even if someone flips motiv_enabled. It stays
    # false so that enabling the PRICE tilt (a small λ nudge) can never silently promote itself
    # into forced -EV STAKES. Re-enable only after a club-data study of its own.
    motiv_conviction: bool = False

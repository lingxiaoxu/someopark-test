...
from prediction_market_soccer.strategy.sizing import size_position
from prediction_market_soccer.strategy.xv_monitor import compare_champion
...
def generate_match_signals(conn=None, *, limit: int = 12) -> tuple[list[MatchSignal], str, bool]:
    ...
    # Per-competition gate (§3.5) — resolved per signal below; this pooled value is
    # only the fallback for a row that carries no league.
    ...
    for m in rows:
        ...
        from prediction_market_soccer.model.probability_calibration import gate_open_for
        gate_open = gate_open_for(cal, m.get("league")) if m.get("league") else gate_open
        ...
        # de-vigged market probability of the side we picked, from the venue we would
        # execute on (falls back to the other venue / the book) — the field existed but
        # was hard-coded None, so every signal row published a null market reference.
        _ven = dec.get("venue")
        _dv = ((m.get(_ven) or {}).get("devig") if _ven else None) \
            or (m.get("kalshi") or {}).get("devig") \
            or (m.get("poly_us") or {}).get("devig") or m.get("book_devig")
        sigs.append(MatchSignal(
            market_family=family, fixture_id=m.get("fixture_id"), match=match_str,
            kickoff=m.get("kickoff"), venue=dec.get("venue"), side=side, side_label=label.get(side, side),
            p_model=dec.get("model_prob"), market_devig=(_dv or {}).get(side), ask=ask,
            net_edge=dec.get("net_edge"),

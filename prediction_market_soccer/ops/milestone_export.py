...
The track is CROSS-COMPETITION (one list, 12 competitions), so every match carries its
`league` key and the doc carries `n_by_league`: the same track filtered to the Premier
League must show the Premier League's denominator, and the PIT strength / knockout
stage tests below are only correct when they are told which competition they are in.

    python -m prediction_market_soccer.ops.milestone_export  →  data/output/milestone_marks.json
"""
...
def build(conn=None) -> dict:
    ...
    matches = []
    for fid in fids:
        # league_id is selected so every per-competition lookup below (`_row_comp`, the
        # registry knockout test, the exported `league` badge) has a competition to key
        # on — without it they all silently fall back to the global/None branch.
        fx = conn.execute(
            "SELECT api_id, home_api_id, away_api_id, home_goals, away_goals, kickoff_ts, round, "
            "       status_short, raw_json, league_id "
        ...
        from prediction_market_soccer.ops.settle_bets import frozen_inplay, frozen_pick
        ...
        if mr is not None:
            ...
            # a KO round drops the home-soil edge (neutral final venue). The stage test is
            # registry-driven and comp-scoped: without comp_key it has to union every
            # competition's round vocabulary, which is only well-defined by luck.
            mp = price_match_calibrated(sm, hi, ai, knockout=False, cal=cal,
                                        host_neutral=is_knockout(fx["round"],
                                                                comp.key if comp else None))
        ...
        from prediction_market_soccer.util.pricing import to_cents as _to_cents
        ...
        if settled and bet and entry_c is not None:
            try:
                ...
                sm_pit = (_pit_strength(conn, fx["kickoff_ts"], _row_comp(fx))
                          if fx["kickoff_ts"] else sm)
        ...
        matches.append({
            "fixture_id": fid,
            # The competition badge the PriceTrack card renders (`leagueLabel(m.league)`)
            # — a track spanning 12 competitions is unreadable without it.
    ...
    for m in matches:
        if m["league"]:
            n_by_league[m["league"]] = n_by_league.get(m["league"], 0) + 1
    return {
        "as_of": datetime.now(timezone.utc).isoformat(),
        "n": len(matches),
        # Per-competition denominators: filtering the track to one league must not read
        # the all-competition total as if it were that league's count.
        "n_by_league": n_by_league,
        "milestones": ["PRE", "T15", "T30", "HT", "T60", "T75", "FT"],
        "note_key": "notes.milestones",
...
def main() -> None:
    ...
    print(f"milestone_marks.json: {doc['n']} matches across "
          f"{len(doc['n_by_league'])} competitions {doc['n_by_league']}")
    for m in doc["matches"][:5]:
        b = m["our_bet"]
        mtm = m["mtm"]
        tail = (f"  bet {b['side']} entry {b['entry_cents']}¢ → "
                f"{'WON +' if mtm and mtm['won'] else 'lost '}{mtm['pnl_c']}¢" if mtm else "")
        print(f"  [{m['league'] or '?'}] {m['home']['name']} vs {m['away']['name']}  "
              f"({len(m['marks'])} marks){tail}")

"""[LEGACY / NOT WIRED] ops/reach_round_export.py — the knockout REACH-ROUND product
(plan 20 Part B).

Superseded by ``ops/season_odds_export`` (TRANSFORM_PLAN §2.2), which is the club
mirror of this ladder: per-league {champion / top-N / relegation} plus, for the UEFA
competitions, the {top8, r16, ro8, ro4, finalist} rungs that map onto KXUCLTOP8 /
KXUCLRO16 / … from the league registry. Nothing in refresh_all or live_refresh calls
this file; it is kept as the reference implementation of the reach-round shape (the
frontend component was reused almost unchanged) and to replay the archived WC series.
...
tournament simulation: p_r16 / p_qf / p_sf / p_final, already in soccer_model.json)
against the live Kalshi Yes price, and surface the per-team edge.

    python -m prediction_market_soccer.ops.reach_round_export → data/output/reach_round.json
"""
...
def _load_champion_rows() -> list[dict]:
    ...
    canonical soccer_model.json (output dir, else the synced frontend dir)."""
    for d in (CONFIG.paths.output, CONFIG.paths.frontend_data):
        p = d / "soccer_model.json"
        if p.exists():
            try:
...
def build(conn=None) -> dict:
    from prediction_market_soccer.venues.champion_prices import REACH_ROUND_SERIES, reach_round_cents
    ...
    return {
        ...
        "theta": theta,
        "note": ("LEGACY reference export — the live club ladder is season_odds.json "
                 "(ops/season_odds_export). Per-team REACH-ROUND market (2-way Yes/No — the "
                 "no-draw 'advance' product, distinct from the per-match 90-min 3-way). Model "
                 "prob of reaching each round vs the live venue Yes price; edge = model − "
                 "cheapest price. Both venues list the shape: Polymarket Global (the liquid "
                 "reference) and Kalshi (per-round qualifier markets; its books are thin and "
                 "often crossed, so the mark is taken from the bid/last, and a quote is shown "
                 "only when within 18¢ of Poly). Real-money trading is still gated + $1-capped."),
        "rounds": rounds,
    }

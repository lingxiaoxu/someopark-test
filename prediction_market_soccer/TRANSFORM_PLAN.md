...
| 9.5 | **ClubElo API 早已冻结**(2026-09-01 勘测发现,比 502 严重) | 缓存的 API 日快照逐日比对:**07-06 → 08-31 之间 594 支球队 0 支变化**(而 02-23 → 08-31 有 481 支变化,说明 API 上半年是活的)。8 月中旬起五大联赛每队踢了 2~3 轮,Elo 不可能不动。网站(权威的球队页 vegaJson 与主页/国家页表格一致)显示的是活数据:利物浦 1902 vs API 1910.8,塞维利亚 1695 vs 1625.9,拜仁 2027 vs 2000.9 | 冻结起点以后**所有先验(含 PIT walk-forward)的 Elo 锚都是同一份陈旧快照**;API "成功返回"但数据是错的,比失败更隐蔽 | 用户决定:(a) API 优先、失败才拉网站(已定);(b) 但必须加**冻结检测**——新日期 CSV 与前一日全表逐值相同且期间有比赛 ⇒ 视同失败,走网站;(c) 冻结期的历史先验是否用网站球队页的历史序列重建,单独评估 |
| 9.6 | 网站解析可行性(勘测结论,未实施) | 主页世界表 ~490 支(整数 Elo,与球队页全精度差 <0.5);**国家页的 JS 数组含该国全部俱乐部**(NOR 页有 Aalesund 1364 等主页没有的小俱乐部),正则 `\['<td class="l">…</td>', 'Elo', …\]` 稳定命中。命名与 API 不同(Bayern München/Bayern、AZ/Alkmaar、Internazionale/Inter),经 `_match_elo` 后五大联赛覆盖与 API 持平;欧战仅靠主页不够(欧协联 25 vs 85),需国家页。注册表里欧战球队的 `team.country` 为空(195 支),国家页定位要另想办法(全量 ~55 个欧洲国家页、每页 570KB、可周缓存) | 仅作 API 失败后的回退 | 实施时:`ingest/clubelo_web.py` 解析 JS 数组 → 写成与 API 相同的 `clubelo_<date>.csv`(加 source 标记),下游零改动;只用于"今天";礼貌抓取(UA、1 req/s);历史日期仍走 API 缓存 |

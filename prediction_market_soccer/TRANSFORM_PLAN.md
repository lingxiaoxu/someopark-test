...
> **后续开发计划（2026-09-10，待执行）**：[Soccer PIT、取价、球队身份与冻结账本修复计划](DEV_PLAN_PIT_DATA_INTEGRITY_20260910.md)。逐项保存外部 assessment 核验、两个历史重建陷阱的当前代码结论、12 个代码工作包与隔离验收；不授权改写历史或改动世界杯模板。
...
## 9. Demo 镜像上线后的待处理项(2026-09-01 记录,不藏)

`exec/kalshi_mirror.py` 把 择时(实现) 账本实时镜像到 Kalshi demo 账户(设计与守卫见模块头注释与记忆
`project_soccer_kalshi_demo_mirror`)。上线时确认了三处**镜像与账本可能对不上**的口径差,都已在数据里留痕、
未做掩盖;它们不是 bug,但要作为后续要处理的问题排进计划:

| # | 现象 | 根因 | 影响范围 | 后续处理方向 |
|---|---|---|---|---|
| 9.1 | 同一笔决策,账本"入场¢/离场¢"与镜像成交价不同 | 账本是纸面记录,取**最优场馆**报价(Poly 优先、再 Kalshi 生产盘);镜像只能在 Kalshi **demo** 盘口成交,demo 由做市机器人报价,与生产盘不同(实测同一市场 demo 63/64¢ 时生产盘无挂单) | 永远存在;只影响价格数字,**不影响选边与定额** | `kalshi_mirror` 表 `ledger_*` 与 `avg_fill_c`/`exit_avg_c` 并排,按此对账;若日后接生产账户,差异自然缩小为价差与滑点 |
| 9.2 | 极少数场次账本显示"持有到底",镜像却已中途卖出 | 账本的 smart_exit 要求**整场 ≥3 个里程碑价点**才评估出场(赛后才知道的充分性条件);镜像在第一个触发的里程碑就必须当场决定 | 仅当实时循环之后**漏捕里程碑**(每个有 8 分钟宽限,罕见)且整场 <3 点 | 两个方向择一:账本改为"逐点因果评估"去掉事后条件;或镜像出场也等到第 3 个点之后。需先统计漏捕频率再定 |
| 9.3 | ClubElo 故障日,当天盘前决策的模型少一个输入 | 模型用 ClubElo 的 Elo 做先验锚;故障时"无 Elo 照常构建";账本**几天后冻结**时读的是那天的 CSV(历史可取),两边输入不同,边缘场次可能选边不同 | 仅 ClubElo 故障日 | 见 9.4 的抓取加固;并考虑"当天先验落盘时记录 has_elo,冻结时若能取到 Elo 则**重建当天先验**再冻结",让两边一致 |
| 9.4 | ClubElo 抓取不够健壮(2026-09-01 实测) | 该站**只提供 http**,https 端口不通(连接超时);日期端点当日返回 **502**(其服务器端错误,`/Fixtures` 等其他端点正常)。旧代码 http/https 各 3 次 × 90s,一次 0.5s 的 502 被放大成 230s 等待 | 每个新进程各自等一遍,实时循环被拖慢 | 已做(同日):仅 http(去掉从不通的 https 回退)、5xx 快速失败(实测 0.4s)、连接超时 8s、重试 1 次、10 分钟磁盘冷却标记、退化构建不落永久指纹。**待做**:日期 CSV 只需抓一次,可改由每日 refresh 提前抓并落盘;故障恢复后核对当天先验是否已重建 |
| 9.5 | **ClubElo API 早已冻结**(2026-09-01 勘测发现,比 502 严重) | 缓存的 API 日快照逐日比对:**07-06 → 08-31 之间 594 支球队 0 支变化**(而 02-23 → 08-31 有 481 支变化,说明 API 上半年是活的)。8 月中旬起五大联赛每队踢了 2~3 轮,Elo 不可能不动。网站(权威的球队页 vegaJson 与主页/国家页表格一致)显示的是活数据:利物浦 1902 vs API 1910.8,塞维利亚 1695 vs 1625.9,拜仁 2027 vs 2000.9 | 冻结起点以后**所有先验(含 PIT walk-forward)的 Elo 锚都是同一份陈旧快照**;API "成功返回"但数据是错的,比失败更隐蔽 | 已定并实施(2026-09-02):(a) **网站优先、API 兜底**;(b) 冻结检测 `clubelo_web.is_frozen`——与前一日全表逐值相同 ⇒ API 答案被拒,改用最近网站文件;(c) 冻结期历史先验用球队页历史序列重建(无页的小俱乐部保留冻结值并标注 `api_frozen`),PIT 先验指纹作废重建。**尺度差**:网站历史序列与 API 是同一模型的两次运行(ClubElo 事后重估),05-31 可比 263 支:网站−API 均值 +66.3、sd 46、Spearman 0.94;先验对 Elo 做赛事内 z-score,所以重建日期里无页面的填充球队(api_frozen)按 +66.3 平移到网站尺度(`api_frozen_rescaled`),避免同一文件内两种尺度混用;07-06 之前的 API 文件仍是真 PIT(API 尺度),之后是网站尺度——跨越 07-06 的 60 天 walk-forward 窗口前半段与后半段尺度不同,但每个日期各自 z-score,相对强弱不受影响 |
| 9.6 | 网站解析(**2026-09-02 已实施**,`ingest/clubelo_web.py`;网站优先、API 兜底) | 主页世界表 ~490 支(整数 Elo,与球队页全精度差 <0.5);**国家页的 JS 数组含该国全部俱乐部**(NOR 页有 Aalesund 1364 等主页没有的小俱乐部),正则 `\['<td class="l">…</td>', 'Elo', …\]` 稳定命中。命名与 API 不同(Bayern München/Bayern、AZ/Alkmaar、Internazionale/Inter),经 `_match_elo` 后五大联赛覆盖与 API 持平;欧战仅靠主页不够(欧协联 25 vs 85),需国家页。注册表里欧战球队的 `team.country` 为空(195 支),国家页定位要另想办法(全量 ~55 个欧洲国家页、每页 570KB、可周缓存) | **生产主源**(用户 2026-09-02 定:先网站、失败再 API) | 已实施:主页世界表(HTML,493 支)+ 55 个欧洲国家页 top-25(JS 数组)→ 900 支/72s;球队页 Vega 历史序列(仅部分俱乐部有页)全部落盘 `data/priors/clubelo_web/`(镜像 `~/clubelo_web_backup/`);名字映射到 API 口径 `name_map.json`(规范化/手工/数值邻近/模糊四步,含 ClubElo 事后修订评分的容差);输出仍是 `clubelo_<date>.csv` API 格式 + `.source` 来源标记;`_fetch_clubelo` 顺序:缓存 → 网站(±1 天)→ 历史重建(过去日期)→ API(冻结即拒、改用最近网站文件 `web_stale`);冻结期 07-06 起逐日重建,旧文件留 `.csv.frozen_api`,逐俱乐部来源写 `.provenance.json`;测试 `tests/test_clubelo_web.py`(15 项)。**复核后追加**(2026-09-02 夜):网站只对 `as_of==今天` 抓取(昨天走历史重建、未来日期只返回最新网站文件不落盘);冻结检测改为与 **API 自己的上一份快照**比对(与网站文件比永远"不相同",会把冻结快照当新鲜数据);`_api_reference` 只读 `.frozen_api` 或 API 来源文件;数值邻近映射步审计 4/4 错配已删除;三字母大写 slug(AEK/AIK/PSV/QPR/IDV)曾被当国家链接丢掉,改为按站点国家代码集合判定;原始页 gzip 落盘 `raw/<date>/`(解析器再改可离线重解析);同日抓取单飞锁;CSV 原子写;`rebuild_frozen_period` 永不碰今天;最后兜底链:最新网站文件 → 标记的冻结缓存 → 抛错(无 Elo 构建)。**链路核实**:实盘先验此前无日更调用(停在 08-28),已加进 refresh_all 每日 `build_all`;每日质量闸门 `ops/clubelo_quality.py`(refresh_all 末尾,FAIL 退出码 2)+ 取数点 `validate_daily`;非生产库的 PIT 先验改写临时目录(`pit_out_dir`),测试禁网 |

| 9.7 | 场馆流动性靠一次性采样判断,结论错了(2026-09-02) | demo 盘口在开球前 6 小时采样为空 → 据此预告"巴甲今晚不会成交";实际做市商临近开球挂单,盘前腿 83¢、盘中腿 80¢ **两笔都成交**。同一天的读数还显示法甲/意甲在 **T-40h** 就有盘口(只是价差 9¢ vs 生产盘 1¢)——"何时可交易"是逐赛事、逐场馆的经验问题 | 任何据此做的可交易性判断 | 已实施 `ops/venue_liquidity.py`:按开球倒计时的固定桶(T-48h/24h/12h/6h/3h/90m/45m/20m/10m/3m)采样 demo 与生产两个盘口,一桶一记、幂等,存 `venue_book_probe`;挂在实时循环**窗口检查之前**(远端桶落在无比赛的日子);`--summary` 给出逐赛事×桶的有卖价/有买价比例与中位价差,每日 refresh 写 `venue_liquidity.json`。**同时修掉一个会毒化测量的 bug**:发现层一次限流失败被缓存成空,再被当作"该场无市场"写进表——现在 `_Tickers.index_ok()` 区分"没有市场"与"问不到",后者不记录、下轮重试 |

...
from prediction_market_soccer.model.venue_climate import (altitude_home_log_edge, venue_index,
                                                          venue_log_suppression)
...
# ── venue altitude (static, deterministic; club edition keys on CITY) ─────────
def test_venue_index_ranks_altitude_and_ignores_the_rest():
    assert venue_index(city="La Paz") > venue_index(city="Bogota") > venue_index(city="Medellin")
    assert venue_index(city="La Paz") == 1.0                # 3,640 m saturates the ramp
    assert venue_index(city="Madrid") == 0.0                # below the 800 m table floor
    assert venue_index(None) == 0.0 and venue_index(city="Unknown Town") == 0.0


def test_altitude_home_edge_only_fires_in_the_fitted_band():
    # Only the >=2,800 m band is significant (z=4.63); the middle bands shrink to ~0 and
    # sea level is exactly 0, so wiring this up cannot move a European fixture.
    assert altitude_home_log_edge("La Paz", 1.0) > 0.5
    assert altitude_home_log_edge("Quito", 1.0) == altitude_home_log_edge("La Paz", 1.0)
    assert altitude_home_log_edge("Bogota", 1.0) < 0.05     # 2,640 m — fitted, not significant
    assert altitude_home_log_edge("London", 1.0) == 0.0
    assert altitude_home_log_edge("La Paz", 0.0) == 0.0     # weight 0 → off


def test_symmetric_suppression_is_retired():
    """The club fit contradicts the WC premise (goals are HIGHER at altitude, and the
    effect is anti-symmetric), so the legacy symmetric path returns 0 for every venue."""
    for place in ("La Paz", "Estadio Azteca", "Madrid", None):
        assert venue_log_suppression(place, 1.0) == 0.0

...
_ASKS_K = {"home": 0.40, "draw": 0.28, "away": 0.40}
_ASKS_P = {"home": 0.41, "draw": 0.29, "away": 0.41}
_DEV = {"home": 0.36, "draw": 0.28, "away": 0.36}
_FORM = {"home_z": 0.0, "away_z": 0.0}


def _receipted(provider, asks):
    # bf13bb71: _decision_quotes runs every side through qualified_quotes — a bare
    # {"ask": x} float has no QuoteReceiptV1 and is no longer executable evidence, so
    # the same asks are issued through the module's own receipt path. Timestamps come
    # from the real clock because qualify_quote enforces max_age against now.
    ...
    fixture = {"api_id": 900, "comp": "ucl", "season": 2026,
               "home_api_id": 1, "away_api_id": 2, "home_id": "club_h", "away_id": "club_a",
               "kickoff_ts": (now + timedelta(hours=2)).isoformat()}
    quotes = {}
    for side, ask in asks.items():
        binding = make_binding(fixture=fixture, provider=provider, environment="prod",
                               event_id="EV900", market_id=f"M-{provider}-{side}", side=side)
        receipt = make_receipt(binding, ask=ask, bid=max(0.01, ask - 0.02),
                               request_started_at=(now - timedelta(seconds=2)).isoformat(),
                               received_at=now.isoformat(), raw={"ask": ask})
        quotes[side] = quote_from_receipt(receipt)
    return quotes
...
def test_knockout_permatch_is_3way_draw_allowed():
    ...
    kq, pq = _receipted("kalshi", _ASKS_K), _receipted("poly_us", _ASKS_P)
    dk = UE._decision_for(_MODEL, kq, pq, _DEV, _DEV, _FORM, 0.25, True, knockout=True)
    dg = UE._decision_for(_MODEL, kq, pq, _DEV, _DEV, _FORM, 0.25, True, knockout=False)
...
def test_knockout_respects_gate():
    kq, pq = _receipted("kalshi", _ASKS_K), _receipted("poly_us", _ASKS_P)
    d = UE._decision_for(_MODEL, kq, pq, _DEV, _DEV, _FORM, 0.25, False, knockout=True)

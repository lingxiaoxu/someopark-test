...
from prediction_market_soccer.venues.guard import VenueGuardError, assert_executable, is_executable
from prediction_market_soccer.venues.kalshi.auth import auth_headers, sign
...
def test_season_mark_uses_bid_on_crossed_book():
    """Regression: season books are thin and often CROSSED — a stale 1¢ sell order can sit far
    below the live bid (the WC case: Argentina→QF ask $0.01, bid $0.60, last $0.63). The mark
    ...
    from prediction_market_soccer.venues.champion_prices import _real_price
...
def test_settled_season_market_uses_result_not_stale_last():
    """Regression: Kalshi nests FINALIZED per-team markets under the still-open season event, so
    list_events(status='open') returns them with an empty live book (ask $1.00 / bid $0.00) and a
    STALE last-trade. The mark MUST come from the settlement result, not that last trade — the
    WC case was Morocco RO16 finalized-YES, book empty, last=$0.37 → 100¢, not a fake 37¢; the
    club analogue is a relegated club's KXEPLRELEGATION market."""
    from prediction_market_soccer.venues.champion_prices import _market_cents
    # the exact finalized-YES shape (dead book, stale last).
    assert _market_cents({
        "status": "finalized", "result": "yes",
        "yes_ask_dollars": "1.0000", "yes_bid_dollars": "0.0000", "last_price_dollars": "0.3700",
    }) == 100.0
    # a survivor's settled-NO market → 0¢ (not its stale last trade either).
    assert _market_cents({
    ...
    assert _market_cents({"status": "finalized", "result": "", "last_price_dollars": "0.40"}) is None
...
def test_polymarket_global_book_parse():
    ...
    from prediction_market_soccer.venues.polymarket_global.reader import parse_clob_book
    payload = {
...
def test_token_bucket_refill_with_fake_clock():
    from prediction_market_soccer.venues.ratelimit import TokenBucket, KalshiRateLimiter, CANCEL_COST
    t = [0.0]
...
def test_hard_dollar_cap_on_orders():
    from prediction_market_soccer.venues.kalshi.orders import enforce_order_cap, OrderCapExceeded

...
def find_opportunities(conn=None, sm=None, *, quote_sources: dict | None = None,
    ...
    live = conn.execute(
        "SELECT api_id, league_id, home_api_id, away_api_id, home_goals, away_goals, elapsed, round, status_short "
        "FROM fixture WHERE status_short IN ({})".format(",".join("?" * len(_LIVE))), _LIVE).fetchall()
    opps: list[Opportunity] = []
    quotes_by_fixture = {}
    for fx in live:
        if fx['status_short'] not in ('1H','HT','2H','LIVE'):
            continue   # regulation contract cannot open after its settlement period
        ...
        score = f"{gh}-{ga}"
        lp, fair = live_fair(conn, fixture_sm, fx)
        ...
        quotes = collect_qualified_sources(quote_sources, fx["api_id"], comp=expected_comp.key if expected_comp else None)

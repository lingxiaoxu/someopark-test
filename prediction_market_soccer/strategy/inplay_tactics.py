...
def live_odds_crossval(*, model_fair: float, book_prob: float | None, side: str,
                       ...
                       move_threshold: float = CROSSVAL_VENUE_MOVE) -> TradeAction:
    ...
    # The book-leads branch trusts the book over our calibrated model, so it carries
    # its own (higher) bar rather than the venue bar plus a fixed step.
    if book_prob - model_fair >= CROSSVAL_LEAD_MOVE:
        return TradeAction("BUY", side,
...
def knockout_late_draw(lp: LiveMatchProb, *, knockout: bool,
                       caps=None, comp_key: str | None = None) -> TradeAction:
    """Stance on a late LEVEL scoreline, from the fitted persistence of this format.

    The World Cup rule was "in a knockout the 90' draw pays and both sides settle for
    extra time, so BACK the draw". Fitted on club data against a per-(competition,
    season) Skellam baseline, that sign is wrong for the format it was aimed at: a
    two-legged decider resolves level states 31% MORE often than chance (n=185,
    z=-3.58) — the tie has to be broken, so somebody goes for it. Domestic league
    games are the ones that drift (1.04, z=+1.26, not significant → stand aside).

    So the decision is delegated to model.knockout_late_draw, which holds the fit and
    shrinks every non-significant family to neutral. ``knockout`` alone is kept as the
    fallback when no caps are supplied, and it now yields a stand-aside rather than the
    inverted WC trade."""
    from prediction_market_soccer.model.knockout_late_draw import late_draw_stance
    st = late_draw_stance(caps, minute=lp.minute, home_goals=lp.home_goals,
                          away_goals=lp.away_goals, comp_key=comp_key)
    if st.direction == "back":
        return TradeAction("BUY", "draw", st.reason, "high",
                           reason_key="late_draw_back",
                           reason_args={"min": lp.minute, "fam": st.family,
                                        "p": round(st.persistence, 3)})
    if st.direction == "fade":
        return TradeAction("SELL", "draw", st.reason, "high",
                           reason_key="late_draw_fade",
                           reason_args={"min": lp.minute, "fam": st.family,
                                        "p": round(st.persistence, 3)})
    return TradeAction("HOLD", "draw", st.reason)
...
_CORNER_ALARMED: set = set()


def _corner_alarm(line: float, edge: float) -> None:
    """A corner edge this wide is a fault report, not an opportunity.

    Every recorded club firing sat between 0.16 and 0.46 on a single fixture and a
    single side — the shape of a model-vs-book structural disagreement, not of a
    market that keeps misprising corners. Trading it would repeat one wrong view many
    times, so it is refused; but it is printed once per line so the corner prior gets
    re-derived instead of quietly producing nothing.
    """
    key = round(line, 1)
    if key not in _CORNER_ALARMED:
        _CORNER_ALARMED.add(key)
        print(f"[corner] model/book gap {edge:.2f} on line {line} exceeds the sanity cap "
              f"{CORNER_EDGE_ALARM} — treating as a prior fault, not an edge")
...
def corner_total_signal(
    ...
    *,
    ...
    from prediction_market_soccer.model.inplay_corners import CORNER_TOTAL_PRIOR, live_corners_fair
...
if __name__ == "__main__":
    ...
    from prediction_market_soccer.model.inplay import live_from_strength

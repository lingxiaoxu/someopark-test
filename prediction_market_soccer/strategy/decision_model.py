...
from prediction_market_soccer.config.config import CONFIG, DecisionConfig, RiskConfig
...
def decide(
    ...
    *,
    ...
    for side in _SIDES:
        ...
        # ABSURD-EDGE GUARD: a claimed edge this far above a quoted market is far
        # more likely to be OUR error than a mispricing. Every early club bet with
        # an edge of +0.25..+0.30 sat on a European qualifier whose club prior was
        # built from a handful of cup games — all of them lost. The prior itself is
        # fixed upstream; this stays as the standing guard against the next
        # thin-prior blow-up.
        max_edge = getattr(cfg, "max_net_edge", 0.0)
        if max_edge and er.net_edge > max_edge:
            continue

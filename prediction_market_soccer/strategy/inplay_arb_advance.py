...
from prediction_market_soccer.strategy.inplay_tactics_advance import (  # draw + totals tactics omitted in the advance view
...
from prediction_market_soccer.strategy.inplay_tactics import corner_total_signal  # market-level, shared
...
def _lone_threat(conn, fixture_row, *, min_team_shots: int = 5):
    ...
    for r in conn.execute(
        ...
        # carded player, for a substitution the one going OFF (`assist` is the one coming
        # ON). Checked against is_starter over every unambiguous substitution on record:
        # 1414 of 1414 had `player` as the STARTER, and a starter cannot be substituted ON.
        # The 3-way twin was corrected earlier and this copy was missed — the same
        # fix-one-forget-the-other that inplay_constants was created to stop.
...
def _tie_shape(conn, fixture_row):
    """(StageCaps, first-leg-only carry) for this fixture, or (None, None).

    Same resolution the exporter uses, so the two cannot drift apart again."""
    try:
        from prediction_market_soccer.config.leagues import by_api_id, caps_for
        from prediction_market_soccer.ingest.soccer_ingest import carry_of
        lid = fixture_row["league_id"] if "league_id" in fixture_row.keys() else None
        comp = by_api_id(lid) if lid is not None else None
        if comp is None:
            return None, None
        leg, carry = carry_of(conn, fixture_row["api_id"])
        rnd = fixture_row["round"] if "round" in fixture_row.keys() else None
        return caps_for(comp.key, rnd, leg=leg), carry
    except Exception as e:   # noqa: BLE001 — shape lookup must not sink the scan
        print(f"[inplay_arb_advance] tie shape lookup failed: {type(e).__name__}: {e}")
        return None, None
...
def live_fair_advance(conn, sm, fixture_row) -> tuple[LiveAdvanceProb, dict]:
    ...
    # The tie's own shape, resolved exactly as ops/inplay_export_advance does. This
    # function used to hardcode host_neutral=True and ignore both the first-leg
    # aggregate and the competition's tie-break rule, so the SAME JSON block carried a
    # header probability from one model and relative-value rows from another — on a
    # real Sudamericana second leg the header read 2.7¢ for home while the row beside
    # it published a 96.5¢ fair and a BUY. One fixture, one model.
    _caps, _carry = _tie_shape(conn, fixture_row)
    _neutral = bool(_caps.neutral) if _caps is not None else True
    lam_h, lam_a = sm.pair_lambdas(hi, ai, knockout=True, host_neutral=_neutral)
    ...
    # Carry the FIRST leg only (carry_of, not the running aggregate) onto tonight's score.
    if _caps is not None and _caps.two_leg and _carry:
        try:
            a_a, a_b = (int(x) for x in _carry.split("-"))
            gh, ga = gh + a_b, ga + a_a   # tie.team_a = leg-1 home = tonight's away side
        except ValueError:
            pass
    ...
    lap = live_advance_prob(
        ...
        # UEFA plays extra time before penalties; CONMEBOL goes straight to the shootout.
        et_then_pens=(bool(_caps.et_then_pens) if _caps is not None else True),
    )
...
def find_opportunities_advance(conn=None, sm=None, *, quote_sources: dict | None = None,
    ...
    # league_id is needed to resolve the tie's shape (and the same omission in the
    # 3-way scanner silently disabled a whole tactic for a day); the league filter
    # keeps the scan on our 12 competitions rather than every live match on earth.
    from prediction_market_soccer.config.leagues import active as _active
    _lids = tuple(c.api_football_id for c in _active())
    live = conn.execute(
        "SELECT api_id, league_id, home_api_id, away_api_id, home_goals, away_goals, "
        "elapsed, round, status_short "
        "FROM fixture WHERE status_short IN ({}) AND league_id IN ({})".format(

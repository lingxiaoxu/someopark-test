"""[LEGACY / NOT WIRED] Monte-Carlo tournament simulation — 2026 48-team World Cup
format (plan 03 §5).

Superseded in the club edition by ``model/league_season`` (season Monte-Carlo:
title / top-N / relegation) and ``model/ucl_phase`` (Swiss league phase → seeded KO
tree), which inherited this file's vectorised skeleton, its ``future_opp`` mechanism
and its "mathematically eliminated" idea. Nothing in the daily or live pipeline calls
``simulate`` any more — only the equally-legacy ``ops/knockout_export``,
``model/ensemble`` and ``model/golden_boot`` do (TRANSFORM_PLAN §2.2 rows
tournament.py / knockout_export.py). Kept for reference and for replaying archived
World Cup priors; the format description below is the WC format it implements, NOT
how any club competition is simulated.
...
from prediction_market_soccer.ingest.club_prior import PriorSnapshot, load_prior
...
from prediction_market_soccer.model import knockout_bracket as _KB
...
def eliminated_teams(conn=None, all_team_ids=None, *, knockout_field_size: int = 32) -> set[str]:
    ...
    Once any knockout fixture exists:
      ...
      * group non-qualifiers = everyone NOT drawn into a knockout tie, but ONLY once the full
        knockout field (``knockout_field_size`` distinct participants, 32 in this legacy
        48-team format) is drawn. API-Football publishes knockout fixtures incrementally, so a partial bracket
...
# these are the reach-probability fields published in soccer_model.json / reach_round.json.

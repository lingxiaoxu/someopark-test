"""[LEGACY / NOT WIRED] 2026 FIFA World Cup knockout bracket — group-position slotting.

NOT part of the club pipeline. No club competition has a 12-group draw with a
best-third revival, so nothing in ``ops/refresh_all`` or ``ops/live_refresh`` reaches
this file; the club knockout path is ``model/league_season`` (seeded KO tree) plus
``model/ucl_phase`` (Swiss league phase) instead. It is kept verbatim because
``model/tournament`` — itself legacy — imports its lookup tables, and because its
constrained-draw ``_perfect_matching`` is the mechanism the UEFA KO-play-off sampler
was modelled on (TRANSFORM_PLAN §3.3). Do not extend it; extend league_season.

...
competitive rather than scripted (in the five one-sided fixtures the out-matched side still wins
11% of matches — e.g. Paraguay beating France 1–0 while conceding 69.5% possession); and the dominant side's possession is
...
pitchfork at β=1 (Thm. 1), removed by feedback gain **g\* = 4(β−1)** (Thm. 2) with a matchup-dependent
set-point (Cor.) whose operating point is *learned* by CEM inside the guaranteed-stable region
(§6.2); intent anchoring guarantees dispersion ≥ (1−λ)²S² (Prop. 2). *Validation* on a
...
(Strogatz 1994), cross-entropy method (Rubinstein & Kroese 2004).
...
set-points `(ρ̂, β, λ)` (the schematic's ρ* denotes this possession target ρ̂ of §6.2, distinct from
the feedback deadband ρ*) + a home-advantage scalar. (2) These set-points are what the control laws
...
system consumes (possession target → the *fixture* target ρ̂ of §6.2, compiled from **both** teams'
...
- **Graph ⊕ LoRA: macro intent meets micro identity.** LoRA shifts a player's *micro* decision
  ...
  gets a sensible tactical prior via shared-concept edges. The KG picks the target ρ̂ and the β, λ set-points; the LoRA picks the
...
Home's recent share is tracked by an EMA `ρₜ₊₁ = (1−α)ρₜ + α bₜ₊₁`, where bₜ₊₁ ∈ {0,1} indicates
whether home wins the next possession contest (eq. 1). Reinforced turnover
...
of measured possession, pass-completion and strength statistics). The compiled target is
`ρ̂(h,a) = clip(½ + κ·[w_p·Δposs + w_c·Δpass + w_z·Δz] + η_home, ½ ± b)` with band b = 0.065 —
realised through the Corollary's mechanism as the closed-loop fixed point ρ† — so Spain–Austria and
Brazil–Japan get *different* calibrations while no two fixtures' targets can diverge by more than
13 points. Enforcement is deliberately soft — *guided emergence*: the target
sets a low-gain director (k_p = 0.4, a coach's nudge rather than a pull) and a weakened
...
behavioural contest* rather than being imposed. The split of labour is visible in the numbers: for
the heaviest mismatches the compiled target saturates at the clip (56.5%) while the emergent seat is
68.2–68.6% (§7) — the target contributes at most 6.5 points of the seat, and the remaining ≈12
points are produced by the behavioural channels acting through the contest. The eight calibration
parameters θ = (κ, w_p, w_c, w_z, lean scale, three style gains) are **learned** by
cross-entropy-method (CEM; Rubinstein & Kroese 2004) policy search over engine rollouts, with
per-fixture loss |emergent share − real-world anchor| + a monopoly penalty + a pass-completion band
penalty. Training uses three strength-diverse fixtures — *Spain–Austria*, *Brazil–Japan* and an
off-corpus *Qatar–Spain* — so **five of the seven evaluation fixtures are held out**, including
both heavy mismatches (*Argentina–Cape Verde*, *Paraguay–France*) behind the venue-symmetry check;
...
`calib_policy.json`; trainer `calibrate_policy.py`). That the learned target leans on style deltas
rather than the raw strength difference (w_z ≈ 0) is expected — possession and passing styles
already encode strength — so the §7 strength-gap scaling is carried through the style channels
rather than a separate strength term. The two §7 observables — the dominant side's seat scaling
smoothly with the strength gap, and its venue symmetry (68.2% home vs 68.6% away, both on held-out
fixtures) — follow from this fixture-dependent, learned policy: the target *plus* the behavioural
gains, not the target alone.
...
is why, in deployment, fixtures seat within a few points of their compiled targets —
Brazil–Japan (target 55%) just past the band edge at 59%, Côte d'Ivoire–Norway (53%) at 52%,
Mexico–Ecuador (50%) at 54%. **β is measured from six open-loop (g = 0) ablation matches** (4,325
...
World-Cup finalists. Per-fixture possession targets are compiled from both teams' real style vectors by the
...
possession scales with the strength gap (per-fixture means 52–60% from the balanced to the
intermediate pairs, up to 68–69% for the heavy mismatches) and stays inside 30–70% in every match; sequences, offsides and recovery track their
...
scripted (Fig. 11): the two balanced fixtures split (Brazil–Japan 3-4-3, Mexico–Ecuador 2-6-2),
while the other five are **one-sided** — a side wins the series by six or more (Spain 7-2-1,
Belgium and Argentina 8-1-1, Côte d'Ivoire 6-3-1, France 8-5-2 *away*). Yet in those five
one-sided fixtures the out-matched side (the series loser) still wins 6 of 55 matches (11%), the
realistic upset rate the stabilisation law makes possible: dominated sides stay in matches instead
of being run over. Overall 36-22-17 to the home side with 149 goals and no blowout artefacts. Shot volume
...
bound (1−λ)²S² that keeps formations intact. The thresholds are analytic, not tuned; what *is*
learned — the per-fixture operating points of §6.2 — moves the system along the stable region those
thresholds carve out, never across it. The lesson generalises: when many
...
≈4000 frames/match; T = 48 teams; distinctness gate 0.3. Calibration policy (§6.2, CEM-learned):
κ = 1.18, w_p = 0.44, w_c = 0.11, w_z = 0.01, anchor band b = 0.065, director gain k_p = 0.4
(hand-set), style gains press 0.69 / directness 0.38 / tempo 0.50 (`calib_policy.json`).
...
NeurIPS · Maher (1982) Stat. Neerl. 36(3) · Park et al. (2023) UIST · Pemantle (2007) Probab. Surveys · Rubinstein & Kroese (2004) Springer

"""
MRPTGenerateReport.py — Paired Backtest vs Validation Period Report Generator

Usage:
    python MRPTGenerateReport.py <backtest_excel> <forward_excel> [output_excel]

Examples:
    python MRPTGenerateReport.py \\
        historical_runs/portfolio_history_all15_best_per_pair_default_20260304_021640.xlsx \\
        historical_runs/portfolio_history_forward30d_best_per_pair_default_20260304_030526.xlsx
...
def _style_cell(cell, bold=False, italic=False, font_color='FF000000',
    ...
    # color_sign: override font color based on cell value (green=positive, red=negative)
...
def _fmt_pct(v):
    """Format percentage as 0-1 float for Excel cell."""
...
def extract_pair_stats(bt_path, fwd_path):
    ...
    def pair_metrics(sheets, pair):
        ...
        if pt.empty or acc.empty:
            return None

        s1, s2 = pair.split('/')
        zcol   = Z_COL_MAP.get(pair, '')
        ...
        # s2 legs — indexed parallel to s1 legs
        ...
        for i in range(n):
            ...
            oz   = float(rv_o[zcol])      if rv_o is not None and zcol in r.columns and not pd.isna(rv_o[zcol]) else np.nan
            ez   = float(rv_o['Entry_Z']) if rv_o is not None and 'Entry_Z' in r.columns else np.nan
            exz  = float(rv_o['Exit_Z'])  if rv_o is not None and 'Exit_Z'  in r.columns else np.nan
            ...
            trades.append({
                ...
                'open_price': s1_open_px, 'close_price': s1_close_px,
                'amount': s1_shares, 'z_at_open': oz, 'entry_z': ez, 'exit_z': exz,
        ...
        if len(opens) > n:
            for i in range(n, len(opens)):
                ...
                rv_o = r.loc[od] if od in r.index else None
                oz   = float(rv_o[zcol])      if rv_o is not None and zcol in r.columns and not pd.isna(rv_o[zcol]) else np.nan
                ez   = float(rv_o['Entry_Z']) if rv_o is not None else np.nan
                # leg details
                s1_shares   = opens.iloc[i]['Amount']
                s1_open_px  = opens.iloc[i]['Price']
                s2_shares   = opens2.iloc[i]['Amount']   if i < len(opens2) else np.nan
        ...
        # z-score stats at open
        z_vals = [abs(t['z_at_open']) for t in trades if not np.isnan(t['z_at_open'])]
        avg_z_open = np.mean(z_vals) if z_vals else np.nan
    ...
    # Also pick up any pairs in the file not in our preset list
    if not bt['pt'].empty:
    ...
    for pair in all_pairs:
        ...
        result[pair] = {
            'bt': bm,
            'fwd': fm,
            'param': PARAM_MAP.get(pair, 'unknown'),
        }
...
def build_summary_sheet(wb, data, bt_port, fwd_port,
    ...
    t = ws['A1']
    t.value = 'Pairs Trading Strategy — Backtest vs Validation Report'
    ...
    port_headers = ['', 'Backtest', 'Validation']
    port_metrics_rows = [
        ...
        for col, val in [(2, bv), (3, fv)]:
            ...
            if typ == 'text':
                ...
                _style_cell(cell, num_fmt='0.0000', align='right',
                            color_sign=cs, border=THIN_BORDER)
            else:
...
def build_pair_sheet(wb, pair, m, bt_start, bt_end, fwd_start, fwd_end):
    ...
    _style_cell(ws.cell(1, 1, f'{pair}  |  Param: {m["param"]}'),
    ...
    metrics = [
        ...
        ('Avg |z| at Entry',     gv(bm,'avg_z_open'), gv(fm,'avg_z_open'), '0.000', False),
    ...
    # Columns: # | Dir | Open Date | Close Date | Hold(d) | z@Open | Entry_Z
    ...
    def trade_table(trades, open_pos, label):
        ...
        for i, t in enumerate(trades):
            ...
            row_data = [
                ...
                (t['hold_days'],   '#,##0',              False),
                (t['z_at_open'],   '+0.000;-0.000',      False),
                (t['entry_z'],     '0.000',              False),
                (ll_sym,           None,                 False),
                (ll_shr,           '#,##0',              False),
                (ll_opx,           '#,##0.00',           False),
                (ll_cpx,           '#,##0.00',           False),
        ...
        # open positions — show entry prices, shares, and current price
        for op in open_pos:
            ...
            open_data = [
                ...
                (None,             None,             False),
                (op.get('z_at_open', np.nan), '+0.000;-0.000', False),
                (op.get('entry_z', np.nan),   '0.000',         False),
                (ll_sym,           None,             False),
                (ll_shr,           '#,##0',          False),
                (ll_opx,           '#,##0.00',       False),
                (ll_cpx,           '#,##0.00',       False),  # current px
                (sl_sym,           None,             False),
                (sl_shr,           '#,##0',          False),
                (sl_opx,           '#,##0.00',       False),
                (sl_cpx,           '#,##0.00',       False),  # current px
                (None,             None,             False),
                (None,             None,             False),
                (None,             None,             False),
            ]
...
def build_daily_pnl_sheet(wb, bt_path, fwd_path, bt_start, bt_end, fwd_start, fwd_end):
    ...
    _style_cell(ws.cell(1, 1, f'Validation Period — Daily PnL by Pair  ({fwd_start.date()} → {fwd_end.date()})'),
                bold=True, font_color=C_HEADER_FG, bg_color=C_HEADER_BG,
    ...
    for ri, (date_str, row) in enumerate(pivot.iterrows(), 3):
        ...
        for ci, val in enumerate(row.values, 2):
            ...
            fg = 'FF000000'
            bg = C_ALT_ROW if alt else None
            if cs and isinstance(val, (int, float)):
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print(__doc__)
        # Default: use latest matching files
        import glob as _glob
        bt_files  = sorted(_glob.glob('historical_runs/portfolio_history_all15_best_per_pair_*.xlsx'))
        fwd_files = sorted(_glob.glob('historical_runs/portfolio_history_forward30d_*.xlsx'))
        if bt_files and fwd_files:
            ...
            print("Usage: python MRPTGenerateReport.py <backtest_excel> <forward_excel> [output_excel]")
            sys.exit(1)
    else:
        bt_path  = sys.argv[1]
        fwd_path = sys.argv[2]

"""
MRPTUpdateConfigs.py — Read Step 1 grid search results and
auto-fill Step 2 (backtest) and Step 3 (forward) config files.

Step 1 runs all 15 pairs × 32 param_sets = 32 runs.
...
Usage:
    python MRPTUpdateConfigs.py [strategy_summary_<ts>.csv]
...
Selection criteria (edit constants below):
    ...
    MIN_DSR    > 0.5    Deflated Sharpe Ratio p-value must exceed 0.5
                        (corrects for 32 trials per pair; DSR > 0.5 ↔ result is
                        statistically meaningful after multiple-comparison penalty)

For each pair:
    - Collect (param_set, pair_pnl, pair_sharpe, dsr_pvalue, n_trades) across all 32 runs
...
def main(csv_path=None, train_start=None):
    ...
    breakdown_path = os.path.join(BASE_DIR, 'historical_runs',
                                  f'grid_pair_breakdown_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv')
    ...
    # Compute trade_start_date for Step 3
    bt_end      = (datetime.now() - timedelta(days=30)).strftime('%Y-%m-%d')
    trade_start = next_weekday(bt_end)
    log.info(f'\nBacktest end:        {bt_end}')
    log.info(f'Forward trade_start: {trade_start}')
    if train_start:
    ...
    step3 = {
        '_comment': (
            ...
            f'trade_start_date={trade_start} (first weekday after backtest end). '
            f'Auto-generated {datetime.now().strftime("%Y-%m-%d %H:%M")}.'
        ),
        'start_date': train_start,
        'end_date': 'auto',
    ...
    log.info('')
    log.info('Next steps:')
    log.info(f'  python PortfolioMRPTStrategyRuns.py {STEP2_PATH}')
    log.info(f'  python PortfolioMRPTStrategyRuns.py {STEP3_PATH}')
    log.info(f'  python MRPTGenerateReport.py <step2_output.xlsx> <step3_output.xlsx>')


if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Update step2/step3 configs from grid search results')
    parser.add_argument('csv', nargs='?', default=None,
                        help='strategy_summary CSV (default: latest in historical_runs/)')

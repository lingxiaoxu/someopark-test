...
def build_pdf(report: dict, output_path: str, yf_compare: bool = True):
    ...
    story.append(Paragraph(
        '未实现盈亏基于报告截止日（end date）的收盘价（yfinance auto-adjusted）；'
        '风险报告基于 signal_date 的收盘价（Polygon），两者相差一个交易日，属正常口径差异。',
        S('_fn', fontSize=6.5, leading=8.5, textColor=C_GRAY, spaceBefore=2,
          alignment=TA_CENTER)
    ))

"""
aiss_adapter — AISS 薄封装(Plan §5.6/§5.8/附录 D;只读输入,只写 outputs/)
=========================================================================
输入(只读): inventory_aiss.json(双层持仓) + account_aiss.json(AUM)
输出: outputs/adapters/aiss_advice_{date}.json (schema_version=v1)
...
def run(date: Optional[str] = None,
    ...
    inv = _read_json(REPO / paths["inventories"]["aiss"])
    acct = _read_json(REPO / paths["accounts"]["aiss"])
    if inv is None:
        warnings.append("inventory_aiss.json missing/unreadable")

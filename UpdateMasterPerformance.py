...
def main():
    ...
    # 4. Merge on common dates
    df = pd.DataFrame({'mrpt': mrpt, 'mtfs': mtfs, 'sr': sr, 'soxx': soxx})
    df['sr'] = df['sr'].ffill()
    ...
    # 5. Compute combined (strategies only) and master (all incl SOXX)
    df['combined'] = df['mrpt'] + df['mtfs'] + df['sr']
    df['master'] = df['combined'] + df['soxx']

    components = ['mrpt', 'mtfs', 'sr', 'soxx', 'combined', 'master']
    records = []
    ...
    # Print summary
    last = records[-1]
    master_start = combined_start * 3  # MRPT+MTFS + SR + SOXX
    print(f'\n  Master start: ${master_start:,.0f}')
    print(f'  Final equities:')
    for comp in components:
        eq = last[f'{comp}_equity']
        start = combined_start if comp in ('sr', 'soxx') else (
            float(df['mrpt'].iloc[0]) if comp == 'mrpt' else
            float(df['mtfs'].iloc[0]) if comp == 'mtfs' else master_start)
        ret = (eq / start - 1) * 100
        dd = last[f'{comp}_dd']
        print(f'    {comp.upper():<8s} ${eq:>12,.2f}  ret={ret:>+7.2f}%  dd={dd:>+6.2f}%')

...
def _freeze_backtest_segment(bt_normalized: pd.Series, key: str) -> pd.Series:
    """拼接点冻结（splice freeze）。

    每个策略的曲线 = 固定回测段 + live 段，拼接点各不相同：
      pairs 3/19（strategy_performance 内部冻结：UpdateSP 只改 --start 之后的行）
    ...
    try:
        with open(MASTER_JSON) as f:
            prev = {r['date']: r.get(f'{key}_equity') for r in json.load(f)}
    except Exception:
        return bt_normalized
    vals, hits = [], 0
    for ts in bt_normalized.index:
        v = prev.get(str(ts.date()))
        if v is not None:
            vals.append(float(v)); hits += 1
        else:
...
def main():
    ...
    sr_bt_normalized = _freeze_backtest_segment(sr_bt_normalized, 'sr')
    ...
    aiss_bt_normalized = _freeze_backtest_segment(aiss_bt_normalized, 'aiss')

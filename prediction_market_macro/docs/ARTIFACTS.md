...
- **坑**：撮合按当日 K 线 close 近似（无深度冲击）；30d/60d 窗口起点不同会导致同名线注数略异。
  **口径注意**：三线对比表里的"大热线（复刻）"跑在选注器数据集上（每事件只取一个 PIT 入场点，
  为与 ML 同轨公平对比），与 Bet 历史"大热线"标签的逐日扫描（daily.streams.argmax）是同一规则的
  两种入场扫描——笔数不同（如 last30 复刻 6 注 vs 实盘口径 16 注）、各自口径内一致，前端已分别标注。

...
| `cpi.py` | **未取整指数**的 MoM 连续性 + 汽油泵价（GASREGW）传导（RB 系数 0.55）+ YoY 精确递推；**0.3.0 起 YoY 双系列 mu 锚定 Cleveland nowcast**（headline 读 `cpi`、core 读 `corecpi`），**0.4.0 起 `predict_mom(core=False)` 的 mu 也整体换成 MoM nowcast**（只动 KXCPI；锚放在 `predict_mom` 而非 `_predict_mom`，所以 `predict_yoy` 走的 MoM 链与 KXPCECORE 逐 bit 不变）。三处 σ 都不动，缺失/陈旧 >7 天回退内部链 | 一个模块喂四个系列；三次锚定的依据各不相同,不许混为一谈：headline-YoY 是 45 事件泄漏免疫回放 Brier −33% 的证据判决，core-YoY 是 44 事件平局后的**用户决定**，headline-MoM 是 62 事件区间 LL 的 OOS 判决（PR-8 / PR-10 有案）；KXCPICORE 的 MoM 锚**被明确否决**。`nowcast_anchor` 键是 PR-8/PR-10 判据的未锚对照臂（默认 `True` 逐 bit 无变化），由 `research/shadow_nowcast.py` 使用，**不进任何选参通道** |
...
| `pit_gates.py` | 回测侧的闸门状态机，与 `decide_all` 读**同一个** `series_enable.SHADOW` 开关。**2026-08-27 (#199)**:`GateHistory` 新增 `params_at`（重放中 params 的**第四种**含义:注册默认 / 全程一组固定 / `params_pit`=生产当时**已采纳**的 / `params_at`=**被测选择器**在那个 asof 会选的）。此前 `walkforward.run` 默认 `db_gates=True` **且** `select_params=True`,于是模拟用选择器的参数**预测**、却用注册默认参数重放它的 skill / capture / conformal 状态——一个从未真实存在过的第三种组合。现由 `_GateBook.gates_for` 把自己的逐日 params 穿进去,`gate_params=True` 为默认（`--no-gate-params` 仅供 A/B,与 `model_exits=False` 同级）。**实测前后对比（生产库克隆,end 2026-08-27）:d30 与 d75 的 headline、逐序列 realized、逐笔成交、以及全部闸门计数全部逐位相同**,只有 `param_rescores`（25→104 / 52→104）与 `params_adopted`（8→13）变化;单序列探针 KXJOBLESSCLAIMS 124 个 asof → 5 个采纳集,52 个收盘里 1 个闸门状态真的动了（skill 1.8554→1.7864）,但两侧都在 `BLOCK_RATIO=1.50` 之上、前后同样是 blocked。**线是活的、今天位移为零**,展示段因此一个数字都没动。刻意**不**接进 `pnl_score.gate_history`（那里按候选重建会循环——闸门拟合在被打分的预测上——且把 73 组网格变成 73 次重放）,正因为那头钉死,这条依赖才是单向且收敛的 |
| `backtest.py` | 预测准确度重放（Brier / CRPS），与交易 PnL 分开 |
| `param_select.py` + `dsr.py` | 每日 **DSR（Deflated Sharpe Ratio）门控的选参**。`MIN_OBS=12`：观测不足就退回注册默认参数，绝不"挑一个看起来最好的"。另含 `manual_params` 覆盖机制（带 PIT 采纳时间戳的历史保留行,三个读取门 `current`/`select_for`/`params_asof` 全部咨询它）。**2026-08-27 修 (#198)**:该读取原先只取**最新一行**再按时间戳丢弃,单次采纳正确、第二次采纳起就错——任何早于最新采纳的 asof 全部退回默认参数（claims 已有 9 行、CPIYOY 5 行）。已改为"取 `created_ts <= before` 的最新一行",`active=False`（`clear_manual`）同样只从自己那一刻起生效。**同日再修 (#198c)**:`_sample_key` 原先只含样本指纹,而写一行 `manual_params` **不结算任何事件**、指纹不动 → `walkforward._GateBook.params_for` 的每日缓存把采纳前的答案一路带过整个采纳期（实测 8 市场 × 4-16 天 = 101 个 (市场,日) 对,覆盖实盘期每一个模拟日）。已把采纳戳并进键;代价是下一次日跑每个市场强制重打分一次,之后自愈。**2026-08-27 加 (#202)**:`param_selection_cache(series, sample_key)` 持久缓存。`param_selection` 按**天**建键,只能服务生产真正站过的那些日子;而 #199 把选参提问搬到**每个受评事件自己的 asof** 上,那些日子生产从未站过,于是每一个都是一次完整网格重打分（实测 ~8.8 秒/次:KXJOBLESSCLAIMS 3.0s→135.3s,整个 30d 跑 40s→6314.7s;而周更在**同一个进程**里要跑约 8 次 `wf.run`——每条 lead 一次 sweep,再 60d,再 30d）。新表改按 `_sample_key` 建键,也就是本模块自己声明的失效契约（样本指纹 + 采纳戳）:两个 `before` 只要键相同,`select_for` 根本分辨不出来——这正是 `refresh` 的跨日 carry-forward 已经依赖的假设,只是把作用域从"同一天"放宽到"任意天"。**与指纹一样,刻意不因代码改动失效**,补救口沿用文档早已写明的 `refresh(force=True)`,并让它**连带清这张表**（否则改完模型跑 force,实验室还会继续吃改动前的答案,而且因为跑得更**快**而毫无察觉）。`refresh()` 顺带把当天的答案播种进去,存的是 **veto 之前**的 `select_for` 原始输出（分支一致性 veto 是生产在其之上的叠加,见 #201）。实测:冷跑 6138.6s 建 104 行,热跑 **59.9s**、0 次重打分 / 104 次命中,且与**完全没有缓存**算出的那一趟 headline、闸门计数、逐序列 realized、逐笔成交**全部逐位相同**——只改时钟不改答案。`gate_stats.param_cache_hits` 是它的探子:表若缺失,`cached_answer` 按 miss 静默降级,而命中数恒为 0 就会暴露 |
...
| `shadow_nowcast.py` | PR-8 / PR-10 的记分器（登记 2026-08-15 / 08-27，落地 08-27，早于第一个前向事件 09-11）。特别之处：锚**已经在生产里**，所以要重建的是**未锚**的对照臂——`model/cpi.py` 加了 `nowcast_anchor`（默认 `True`）。默认值经并排实测逐 bit 无变化（**200/200** 已结算预测的 comps+inputs+data_horizon 完全相同，故 `VERSION` 不 bump），翻掉时 **151/151** 有锚事件全部移动。该键**两条选参通道都不收**——它是判据的对照臂，不是可搜的参数。两个登记内部各有一处不自洽，评分器把两边都算出来并标注哪一条 grades nothing：PR-8 的"两系列合计"对上改道注记的"headline 六"（core 单独计数并单独出判词，因为 core 的接线依据是用户决定不是证据），PR-10 的 T-26h 判据对上发现期用的 close−1h |
...
| **PR-8** | CPI-YoY 族 mu 锚定 Cleveland nowcast | K=1；前向 6 个结算事件,T-26h 配对逐腿 Brier | **登记数小时后被用户指令改道**：改跑判据的历史等价物（45/44 事件泄漏免疫回放）。headline 判决性通过（Brier −33%）→ 已上线；core 平局 → 用户决定也上线（依据=决定非证据,原文有案）。前向计数降级为确认监控（记分器 `shadow_nowcast.py` 2026-08-27 落地，headline 与 core **分开计数、分开出判词**） |
| **PR-9** | demo 账户镜像跟单可以晋级为 arm | K=1；paper 台账 ≥10 笔结算 且事件聚类 bootstrap 95% CI 的 ROI > 0 | 前瞻中（登记时 6/10）。代码以**暗模式**先建，判据门控的是 `arm()` 而不是写代码 |
| **PR-10** | KXCPI 的 headline MoM 分支 mu 锚定 Cleveland **MoM** nowcast | K=12；前向 6 个结算事件,T-26h 配对**区间对数似然** 且 ≥4/6 逐事件为正 | 前瞻中 **0/6**。**首个前向事件 2026-09-11**，记分器 `shadow_nowcast.py` 已在同日落地、赶在第一个事件之前。KXCPICORE 已明确否决（单事件贡献 101.8% 的增益） |
...
| **manual_params 采纳（2026-08-11,生效中）** | 用户明示指令：将 75 天叉积（参数 × 流 × 离场,PnL 打分）在 **prod 规则约束下**（hybrid+离场开）的每市场 argmin 参数采纳进实盘。机制 `param_select.manual_params`：覆盖行优先于日选器、带 PIT 采纳时间戳（早于采纳时刻的模拟日不受影响）、`clear_manual` 一键回退、审计留痕 | **10 市场已写入**（claims/CPI×4/payrolls/U3/Fed/WTIW/NATGASW）,4 市场最优=默认未动（PCECORE/FEDDECISION/AAAGASW/GDP）。**DSR 反对意见在案**：全部市场 n_obs 1-11 < 12,搜索宽度 30-654 格,这些是折减门拒绝采纳的样本内 argmin,按用户指令上线。核对点:暴力全量扫描（`docs/PLAN_BRUTE_SWEEP.md`）完成后回头验证这批采纳在组合真值下是否仍是 argmax。**2026-08-27 更正 (#198)**:"早于采纳时刻的模拟日不受影响"当时是对的（只有一次采纳）,但 argmin 车道此后每周改写,到 08-27 已积到 claims 9 行 / CPIYOY 5 行,而读取器只看最新一行 → **晚于第一次采纳、早于最新一次的那些日子也退回了默认参数**。已修并实测:冻结展示段（`end2026-08-04`,全窗早于 08-11）**0 处分歧,冻结数字一分不动**;滚动 75 天窗有 87 个 (市场,日) 对改变,集中在 claims 15 / CPIYOY 14 / CPI·CORE·COREYOY 各 11 / U3 11 / payrolls 10 / WTIW 4。**再加 #198c**（缓存键不含采纳戳,见 §模块表 `param_select.py` 行）之后,**前端 WF Lab 的滚动行确实会动**:30d hybrid $6.76→**$6.53**、ROI 58.885%→**56.882%**;60d hybrid $14.16→**$13.93**、ROI 58.902%→**57.945%**（argmax 两窗各多出 1 笔）。方向向下是对的——修复前那两个窗在整个实盘期都按**默认参数**模拟,而生产跑的是采纳参数,采纳参数拿到窗口外要还一点回去,跟本表"事后定格 +26.44% vs 每日 PIT 滚动 −25.21%"记的是同一件事(只是量级小得多)。披露留痕在 `docs/PREREGISTER.md`「纪律 4 的执行留痕」 |

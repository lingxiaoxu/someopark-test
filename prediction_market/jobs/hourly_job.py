...
Scheduling (plan 05 §4): run @hourly from cron/launchd, or use ``--loop``.
    ...
    0 * * * * cd /Users/xuling/code/someopark-test && set -a && . prediction_market/.env && set +a && \
      conda run -n someopark_run python -m prediction_market.jobs.hourly_job >> prediction_market/data/logs/hourly.log 2>&1

    # or a self-contained loop (foreground)
    conda run -n someopark_run python -m prediction_market.jobs.hourly_job --loop 3600
"""
...
def run_once(*, dry_run: bool = False, ingest: bool = True, emit_frontend: bool = False,
    ...
    if ingest and not dry_run:
        from prediction_market.ingest import soccer_ingest
        try:
    ...
    from prediction_market.model.run_model import build_payload, write_outputs
    prior = load_prior()
    ...
    xv_top = None
    try:
        from prediction_market.strategy.xv_monitor import compare_champion, write_report
    ...
    sig_summary = None
    try:
        from prediction_market.exec.executor import run as run_signals
        sg = run_signals()
    ...
    inplay_n = 0
    try:
        from prediction_market.jobs.live_poller import poll_once
    ...
    health = None
    try:
        from prediction_market.ops.monitor import health_report
...
def main() -> None:
    ap = argparse.ArgumentParser(description="World Cup hourly pipeline")

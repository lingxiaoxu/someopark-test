...
(Accuracy/PnL, PriceTrack, PnL report) are stable — "a bet, once placed, never changes".
    ...
    python -m prediction_market.ops.settle_bets   →  freeze any newly-settled matches
"""
...
def _pit_py(conn, cmap):
    ...
    for r in rows:
        ...
        if not (hi and ai) or not r["kickoff_ts"]:
            continue
        mp = price_match(_pit_strength(conn, r["kickoff_ts"]), hi, ai)
...
def _inplay_entry(conn, fx_row, hi, ai):
    ...
    from prediction_market.ops.performance_report import _pit_strength
    from prediction_market.strategy.decision_model import _clip, _kelly_fraction
    from prediction_market.strategy.smart_exit import _match_minute, smart_exit_cashout
    from prediction_market.util.pricing import pnl_cents, reg_score
    ...
    sm_pit = _pit_strength(conn, fx_row["kickoff_ts"])

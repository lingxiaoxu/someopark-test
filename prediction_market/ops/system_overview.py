...
INTERFACES = [
    ...
    ("策略", "strategy.decision_model", "赛前下注决策:价值选边 + 分数凯利 + 置信度定额($0.2–$2)"),
...
VALUE = [
    ...
    "一套赛前下注决策模型(非「押最可能」而是「押最被低估」):按价值/edge 选边、分数凯利(¼)定量、"
    "置信度($0.2–$2)缩放,并对冷门高估偏差设更高门槛;全程点对点(PIT)信息、无未来泄漏,真实下单仍受 $1 硬顶。"
    "以闭线价值(CLV)为首要成绩,argmax 预测准确率作模型质量参考。",

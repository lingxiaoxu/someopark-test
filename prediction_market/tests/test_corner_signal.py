...
Run: python -m pytest prediction_market/tests/test_corner_signal.py -q
"""
import pytest

from prediction_market.model.inplay_corners import live_corners_fair
from prediction_market.strategy.inplay_tactics import corner_total_signal
...
def test_model_overdispersion_fatter_in_deep_tail():
    ...
    from prediction_market.model.inplay_corners import live_corners_fair as f
...
def test_signal_fires_over_on_real_edge():
    # live-hot game (10 corners by 55') vs a market pricing the 9+ line cheap
    a = corner_total_signal(10, 55, 1, 1, quotes={8.5: {"ask": 0.40, "bid": 0.38}})
    assert a.act == "BUY" and a.side == "over"
    assert a.reason_key == "corner_value" and a.reason_args["edge"] >= 0.07


def test_signal_fires_under_on_real_edge():
    # quiet game (2 corners by 60') but the market prices the OVER expensive → UNDER value
    a = corner_total_signal(2, 60, 0, 0, quotes={8.5: {"ask": 0.60, "bid": 0.58}})
...
def test_signal_picks_largest_edge_line():
    ...
    q = {7.5: {"ask": 0.55, "bid": 0.53}, 8.5: {"ask": 0.30, "bid": 0.28}}

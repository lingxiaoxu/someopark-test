...
Run:  conda run -n someopark_run python -m pytest prediction_market/tests/test_inplay_advance_model.py -q
  or: conda run -n someopark_run python prediction_market/tests/test_inplay_advance_model.py
"""
from __future__ import annotations

from prediction_market.model.penalties import (
...
def _sm():
    ...
    return build_strength(load_prior())
...
def test_advance_probs_sum_to_one_every_state():
    ...
    for st in states:
        la = live_advance_from_strength(sm, "brazil", "japan", **st)
...
def test_leading_increases_advance_prob():
    from prediction_market.model.inplay_advance import live_advance_from_strength
    sm = _sm()
    base = live_advance_from_strength(sm, "brazil", "japan", 60, 0, 0).p_home_advance
    lead = live_advance_from_strength(sm, "brazil", "japan", 60, 1, 0).p_home_advance
    trail = live_advance_from_strength(sm, "brazil", "japan", 60, 0, 1).p_home_advance
...
def test_hedge_full_locks_equal_payoff():
    from prediction_market.strategy.inplay_hedge_advance import (
...
def test_hedge_dutch_lock_detects_underround():
    from prediction_market.strategy.inplay_hedge_advance import Quotes, dutch_lock
...
if __name__ == "__main__":
    ...
    from prediction_market.model.inplay_advance import live_advance_from_strength
    sm = _sm()
    print("shootout conversions Brazil/Japan:", shootout_conversions(sm, "brazil", "japan"))
    print("detailed shootout P(Brazil):", round(shootout_win_prob_detailed(sm, "brazil", "japan"), 3))
    print("\n=== Virtual match: Brazil vs Japan — live P(Brazil advances) ===")
    timeline = [
        (10, 0, 0, "reg"), (30, 0, 0, "reg"), (55, 0, 1, "reg"),   # Japan shock lead
        (70, 1, 1, "reg"), (89, 1, 1, "reg"),                      # Brazil equalises, level late
    ...
    for mn, gh, ga, period in timeline:
        ...
        la = live_advance_from_strength(sm, "brazil", "japan", mn, gh, ga, **kw)
        print(f"  {mn:>3}' {gh}-{ga} [{period:>4}]  P(BRA adv)={la.p_home_advance:.3f}  "

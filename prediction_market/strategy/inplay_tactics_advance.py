"""In-play trading tactics — 2-WAY "ADVANCE" FORK (plan 24 §3) of inplay_tactics.py.

Knockout who-advances twin of strategy/inplay_tactics.py. Operates on the live ADVANCE model
(model/inplay_advance.LiveAdvanceProb: p_home_advance / p_away_advance, incl. ET + penalties)
— NO draw leg. Forked from the 3-way file by: DELETED draw-only signals (draw_trade_signal,
knockout_late_draw); RESULT fairs read the 2-way advance probs (_fair → home/away advance);
RECALIBRATE lock/overshoot thresholds (home/away absorb the old draw mass — constants kept but
flagged); TOTALS + event/stat signals copied as-is (settlement-agnostic; read regulation
totals/state carried on LiveAdvanceProb). The 3-way file is UNCHANGED and runs in parallel.
Original 3-way docstring follows.
---
...
from prediction_market.model.inplay_advance import LiveAdvanceProb

# Defaults (calibrate from data; plan 04 §4c keeps these conservative).
# RECALIBRATE for 2-way (plan 24 §3): in the advance market home/away absorb the old draw
# mass, so fair values run higher — these lock/take-profit thresholds may trigger sooner than
# in 3-way. Kept at the 3-way values for now; tune against 2-way fairs from replay.
...
def _fair(lp: LiveAdvanceProb, side: str) -> float:
    # 2-way advance: only home/away exist. Draw is never priced (orchestrator skips it).
    return {"home": lp.p_home_advance, "away": lp.p_away_advance}.get(side, 0.0)
...
def convergence_take_profit(side: str, entry_price: float, lp: LiveAdvanceProb, *,
...
def model_overshoot_take_profit(side: str, market_bid: float | None, lp: LiveAdvanceProb, *,
...
# DELETED for 2-way (plan 24 §3): draw_trade_signal — the draw lifecycle (early time-value /
# late-equalizer lock) is a 3-way-only product; in the advance market there is no draw leg.


def totals_time_decay(lp: LiveAdvanceProb, line: float = 2.5, *,
...
def goal_overreaction_fade(lp: LiveAdvanceProb, *, prematch_fav_side: str | None,
    ...
    fav_equity = lp.p_home_advance if prematch_fav_side == "home" else lp.p_away_advance
...
def favourite_comeback(lp: LiveAdvanceProb, *, prematch_fav_side: str | None,
...
def red_card_value(lp: LiveAdvanceProb, *, carded_side: str | None, card_minute: int | None) -> TradeAction:
...
def dormant_explosion(lp: LiveAdvanceProb, *, combined_xg: float | None) -> TradeAction:
...
def finishing_uplift_over(lp: LiveAdvanceProb, line: float = 2.5, *,
...
def formation_fragility(lp: LiveAdvanceProb, *, home_formation: str | None,
...
def late_goal_bias(lp: LiveAdvanceProb, line: float = 2.5, *,
                   entry_under: float | None = None) -> TradeAction:
...
# DELETED for 2-way (plan 24 §3): knockout_late_draw — it backed the 90' DRAW because the
# 3-way KO contract pays "Tie" at 90'. The advance market has no draw and is decided by
# ET/penalties, so the premise is invalid.


if __name__ == "__main__":
    from prediction_market.ingest.prior_ingest import load_prior
    from prediction_market.model.inplay_advance import live_advance_from_strength
    from prediction_market.model.strength import build_strength

    sm = build_strength(load_prior())
    print("ADVANCE tactics — Brazil vs Japan:")
    # Convergence take-profit on a held home-advances position, leading 2:0 at 80'.
    lead = live_advance_from_strength(sm, "brazil", "japan", 80, 2, 0)
    print(f"  80' 2:0  P(home adv)={lead.p_home_advance:.2f}")
    print("  convergence:", convergence_take_profit("home", 0.55, lead, market_bid=0.90))
    print("  overshoot:", model_overshoot_take_profit("home", 0.95, lead, entry_price=0.55))

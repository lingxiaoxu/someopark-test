...
- **冻结常数非活比率**(ratio 纪律不破);官方引擎自身演化(费用/分红/复利)
  与纯持仓收益的楔子会缓慢累积 → 对账单列"官方引擎楔子"项,漂移 >1% 时
  做显式再基准(重算 scalar + 一次调仓,留痕),绝不日常追比率;
- 防火墙注: 官方 perf json 仅 go-live 读一次冻结常数;常驻循环仍只读五持仓文件。
- 收敛结论(用户预期成立): legacy 清零 + K 校准后,QC equity ≈ **面板头条**
  (官方口径 Σ),残余=执行成本流+残差+官方引擎楔子(全部对账可见)。
...
1. **水平差不会自动归零 —— 计划做一次性再基准**。legacy 全死后,两边**日度
   ...
   (~$150)。
2. **对比基准必须是账本口径,不是面板头条**。实时 NAV 面板主数字是
   **官方口径**(官方 EOD × 日内收益,现 ≈$5.98M);QC 镜像的是**账本口径**
   (持仓文件字面 shares,$1M/策略起账,现 ≈$5.07M)。QC equity 永远不该拿去
   跟面板头条比 —— 对账与 M5 面板集成一律对齐 controller 账本口径的
   PORTFOLIO 值(面板 tooltip/内部值)。两口径换算是展示层的事,不进执行。

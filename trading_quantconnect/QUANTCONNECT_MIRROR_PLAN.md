...
| 时点(ET) | 动作 |
|---|---|
| 周一 ~10:00 | **早查 QC 成交**(预先准备):applied v17、20 腿收敛、无拒单、cash ~$1.6M 量级;命令见 EVENT_RISK §13.5 周一块 |
...
**✅ 执行记录(2026-09-01)**:官方 EOD 提前落地(10:09 已见 8/31 行)→ 手动
settle:③ **ok**,ΔD +20,301.70 = 镜像滞后 +5,454.98 + 滑点 +14,941.73 + 其他,
残差 −679.58 = **−1.25bp**(20 笔成交,阈值 3bp;含预告的 +108 重放余波)→
10:10 ET `--freeze --session 2026-08-31` 执行,**K = −12,004.77**,rolloff.json
逐位 == 报告(provenance=m4_report_anchored)。冻后 --measure:K_eff = 冻结值 +

...
```bash
# 双策略合并运行（推荐）
export $(cat .env | xargs) && conda run -n someopark_run python DailySignal.py \
  --strategy both --total-capital 1000000

# dry-run（只看信号，不写 inventory）
export $(cat .env | xargs) && conda run -n someopark_run python DailySignal.py \
  --strategy both --total-capital 1000000 --dry-run

# 单策略 / 跳过 Regime 检测（离线时）
export $(cat .env | xargs) && conda run -n someopark_run python DailySignal.py --strategy mrpt
export $(cat .env | xargs) && conda run -n someopark_run python DailySignal.py --strategy both --skip-regime
```

**主要功能：**

- **Regime 检测**（`RegimeDetector.py`）：综合波动率、信用利差、利率、AI 动量等 7 类市场指标，输出 0–100 分，自动决定 MRPT / MTFS 资金权重。分数低（< 40）偏均值回归，分数高（> 60）偏趋势跟随，中间为中性
- **Position Monitor**：对所有已开仓配对，单独以开仓时的 param_set 跑一次专属回测，计算当日止损状态和未实现盈亏，独立于新信号，输出 HOLD / CLOSE / CLOSE_STOP
- **完整报告**：所有输出统一写入 `trading_signals/`，包含信号 JSON 和人类可读中文 TXT 报告（含 Regime 分析、持仓监测、OOS 历史参考）

持仓快照保存在 `inventory_mrpt.json` 和 `inventory_mtfs.json`，每条记录含开仓日期、价格、使用的 param_set 等信息，供 Position Monitor 复现开仓参数。

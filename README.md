...
| `VIXForecast.py` | VIX Chronos-2 预测模块：双模型集成（finetune-full + finetune-fomc），预测未来 10 交易日 VIX 走向，输出 0.15–0.85 score 供 RegimeDetector 使用。context=504天，支持 VIX9D/VIX3M past_covariates + FOMC future_covariates，checkpoint 当日复用 |
...
| `RefreshBDCHoldings.py` + `SyncPrivateCreditRates.py` + `RunBDCLookThrough.py` | **BDC Look-Through 日管道（私募信贷透视，16:05 ET）**：真实 SEC SOI（GBDC/TSLX/OBDC/BXSL/ARCC 5 家底层贷款，~4,530 tranche）逐日重估值 + 季度 diff + mark 恶化预警。EDGAR 双通道（A=月度 BDC Data Sets zip / B=inline-XBRL 解析，**channel A 404 在财报季后 4-8 周属发布节奏预期**）；摄取层单位守卫（利率百分点、principal $000s → 大声告警归一）；到期日 96.6% 真实提取（MM/YYYY+MM/DD/YYYY，其余 imputed 有标注）。与股票层 `UpdateBDCPerformance`（yfinance 实盘价）完全独立。详见 RUNBOOK §6.6 |
| `portfolio_of_private_credit_deals/bdc_stress.py` | **§7.3 情景压测（2026-08-15 上线，STEP D 内每日运行 ~85s）**：利率梯度 ±100/200/300bp + 三宏观情景（mild/severe recession、stagflation）。survival-weighted 期望现金流、mark 入场（−FV）、**mark 锚定隐含利差贴现**（逐笔解 y* 使 base PV=公允价 → base ΔEV≡0，情景=y*+Δfwd+widen）；PD_s 消费逐笔 score 压力乘数（0.8–2.0）分层；PIK 终偿 balloon 入流。Floor 出处分级：BXSL 逐笔（SOI 脚注 0.50–3.00%）/ GBDC 0.76%、OBDC 0.80% 披露 wavg / ARCC、TSLX 份额披露+1.00% ASSUMED。产出 `stress` 块（10 情景×8 指标 + by_bdc 归因 + base↔headline bridge）进日报与心跳（`stress_worst=`）。**纯分析层，不碰实盘记账** |
...
| 窗口 | 训练期 | 测试期（样本外） |
|------|--------|----------------|
| Window 1 | 2024-03-11 → 2025-09-10 | 2025-09-11 → 2025-10-15 |
| Window 2 | 2024-03-11 → 2025-10-15 | 2025-10-16 → 2025-11-19 |
| Window 3 | 2024-03-11 → 2025-11-19 | 2025-11-20 → 2025-12-26 |
| Window 4 | 2024-03-11 → 2025-12-26 | 2025-12-29 → 2026-02-03 |
...
| 窗口 | 训练期 | 测试期（样本外） |
|------|--------|----------------|
| Window 1 | 2024-02-23 → 2025-08-22 | 2025-08-25 → 2025-10-01 |
| Window 2 | 2024-02-23 → 2025-10-01 | 2025-10-02 → 2025-11-07 |
| Window 3 | 2024-02-23 → 2025-11-07 | 2025-11-10 → 2025-12-17 |
| Window 4 | 2024-02-23 → 2025-12-17 | 2025-12-18 → 2026-01-28 |
| Window 5 | 2024-02-23 → 2026-01-28 | 2026-01-29 → 2026-03-09 |
| Window 6 | 2024-02-23 → 2026-03-09 | 2026-03-10 → 2026-04-16 |
...
# 跳过 OOS 重跑（复用已有结果，自动验证 tail 覆盖和 NaN，不合格则重新跑）
python MRPTWalkForward.py --skip-oos
```
...
# 跳过 OOS 重跑（同 MRPT）
python MTFSWalkForward.py --skip-oos
```
...
| `trading_signals/pnl_reports/` | PDF 绩效报告（`PnLReport.py` 输出，文件名 `pnl_report_YYYYMMDD.pdf`） |
